...
def compute_reconciliation_actions(open_sessions, currently_present):
    ...
    Discord access, just dict/set math, so it's fully unit-testable.
    ...
    Returns (to_close, to_open) as sets of user_ids:
      to_close = users whose open session should be closed now — either
                 they're no longer present at all (bot missed their LEAVE
                 while offline), OR they're present but in a DIFFERENT
                 channel than their open session says (bot missed a MOVE,
                 or a LEAVE+JOIN pair, while offline — the stale channel's
                 session must be closed even though the user never fully
                 left voice).
      to_open  = users who need a new session opened at their CURRENT
                 channel — either newly discovered present with no open
                 row at all, or the same channel-mismatch case above (a
                 user_id can legitimately appear in BOTH sets when they
                 moved channels while the bot was offline: close the old
                 channel's session, open the new channel's session).
    ...
    for uid, open_channel_id in open_sessions.items():
        if uid not in currently_present:
            to_close.add(uid)  # left entirely while offline
        elif currently_present[uid] != open_channel_id:
            to_close.add(uid)  # moved to a different channel while offline
            to_open.add(uid)

    for uid in currently_present:
        if uid not in open_sessions:
            to_open.add(uid)  # newly present with no session on record at all
