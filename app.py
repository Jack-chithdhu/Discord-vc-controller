...
def init_db():
    ...
    try:
        ...
        print("[db] PostgreSQL ready (vc_logs, vc_lock_channels, vc_lock_members, join_requests, bot_settings, birthdays, points, vc_sessions).", flush=True)
        return True
...
async def reconcile_vc_sessions():
    ...
    vc_sessions against REAL Discord presence, in both directions:
    ...
    The close/open decision itself is pure set math, tested in isolation —
    see vc_stats.compute_reconciliation_actions. This function's only job
    is gathering the two real-world sets and applying the result.
    """
    if not _pool:
        return
    guild = bot.get_guild(GUILD_ID)
    ...
    with _cursor() as cur:
        cur.execute("SELECT user_id FROM vc_sessions WHERE ended_at IS NULL")
        open_user_ids = {r["user_id"] for r in cur.fetchall()}

    currently_present = {}  # user_id (str) -> (channel_id, channel_name)
    for vc in guild.channels:
        if isinstance(vc, discord.VoiceChannel):
            for m in vc.members:
                currently_present[str(m.id)] = (vc.id, vc.name)

    to_close, to_open = vs.compute_reconciliation_actions(open_user_ids, set(currently_present.keys()))

    now = datetime.now(timezone.utc)
    for uid in to_close:
        close_session(uid, at=now, end_is_estimated=True)
    for uid in to_open:
        channel_id, channel_name = currently_present[uid]
...
async def on_voice_state_update(member, before, after):
    ...
    if before_id is None and after_id is not None:
        ...
        open_or_move_session(member.id, member.display_name, after.channel.id, after.channel.name)
        return
