...
def start_bot():
    ...
    while True:
        ...
        try:
            ...
            print("[bot] Discord connection closed cleanly, not reconnecting.")
            break
        except discord.LoginFailure:
            print("[bot] DISCORD_TOKEN is invalid — fix the token and redeploy. Not retrying.")
            break
        except discord.HTTPException as e:
            status = getattr(e, "status", None)
            if status == 429:
                print(f"[bot] Discord rate-limited our login (429). Retrying in {retry_delay}s...")
            else:
                print(f"[bot] Discord HTTP error during login ({status}): {e}. Retrying in {retry_delay}s...")
        except Exception as e:
            print(f"[bot] Unexpected error connecting to Discord: {e}. Retrying in {retry_delay}s...")
