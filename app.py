...
<body>
  ...
  <div class="view" id="view-settings">
    ...
    <h2 style="margin-top:20px;">VC Reservation limits</h2>
    <div id="reservationLimitsForm" style="display:flex; flex-direction:column; gap:8px; max-width:320px;">
      <label class="muted" style="font-size:13px;">Cooldown between requests (minutes)
        <input type="number" id="resvCooldown" min="0" style="width:100%; margin-top:2px;"></label>
      <label class="muted" style="font-size:13px;">Max pending requests per user
        <input type="number" id="resvMaxPending" min="0" style="width:100%; margin-top:2px;"></label>
      <label class="muted" style="font-size:13px;">Max reservations per day per user
        <input type="number" id="resvMaxPerDay" min="0" style="width:100%; margin-top:2px;"></label>
      <label class="muted" style="font-size:13px;">Max active reservations per user
        <input type="number" id="resvMaxActive" min="0" style="width:100%; margin-top:2px;"></label>
      <label class="muted" style="font-size:13px;">Pending request expires after (hours)
        <input type="number" id="resvExpireHours" min="0" style="width:100%; margin-top:2px;"></label>
      <label style="display:flex; align-items:center; gap:8px; font-size:13px;">
        <input type="checkbox" id="resvSpamEnabled"> Spam protection enabled</label>
      <label style="display:flex; align-items:center; gap:8px; font-size:13px;">
        <input type="checkbox" id="resvDupEnabled"> Duplicate/overlap protection enabled</label>
      <label style="display:flex; align-items:center; gap:8px; font-size:13px;">
        <input type="checkbox" id="resvAutoExpireEnabled"> Auto-expire old pending requests</label>
      <button class="btn small" onclick="saveReservationLimits()" style="width:fit-content;">Save</button>
      <div id="reservationLimitsMsg" class="muted" style="font-size:12px;"></div>
    </div>
  </div>
...
async function refreshCurrentTab() {
  ...
  else if (currentTab === 'settings') { loadToggles(); loadBirthdayReminderInterval(); loadReservationLimits(); }
}
...
async function loadReservationLimits() {
  const data = await apiGet('/api/reservation-settings');
  if (!data) return;
  document.getElementById('resvCooldown').value = data.cooldown_minutes;
  document.getElementById('resvMaxPending').value = data.max_pending;
  document.getElementById('resvMaxPerDay').value = data.max_per_day;
  document.getElementById('resvMaxActive').value = data.max_active;
  document.getElementById('resvExpireHours').value = data.expire_after_hours;
  document.getElementById('resvSpamEnabled').checked = data.spam_protection_enabled;
  document.getElementById('resvDupEnabled').checked = data.duplicate_protection_enabled;
  document.getElementById('resvAutoExpireEnabled').checked = data.auto_expire_enabled;
}

async function saveReservationLimits() {
  const msg = document.getElementById('reservationLimitsMsg');
  const payload = {
    cooldown_minutes: parseFloat(document.getElementById('resvCooldown').value),
    max_pending: parseInt(document.getElementById('resvMaxPending').value),
    max_per_day: parseInt(document.getElementById('resvMaxPerDay').value),
    max_active: parseInt(document.getElementById('resvMaxActive').value),
    expire_after_hours: parseFloat(document.getElementById('resvExpireHours').value),
    spam_protection_enabled: document.getElementById('resvSpamEnabled').checked,
    duplicate_protection_enabled: document.getElementById('resvDupEnabled').checked,
    auto_expire_enabled: document.getElementById('resvAutoExpireEnabled').checked,
  };
  const res = await apiPost('/api/reservation-settings', payload);
  if (res && res.__error) { msg.textContent = res.__error; return; }
...
async function loadLeaderboard() {
  ...
  html += data.vc_time.length
    ? data.vc_time.map((r, i) => `<div class="lb-row"><span class="lb-rank">${medal(i)}</span><a class="lb-name" href="/profile/${r.user_id}" target="_blank" style="color:inherit; text-decoration:none;">${r.user_name} <span style="opacity:.55; font-size:11px;">👤</span></a><span class="lb-value">${r.formatted}</span></div>`).join('')
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0">
<title>VC Profile</title>
<style>
  :root {
    --bg: #12101c; --panel: #1d1a2e; --panel2: #26223a;
    --accent: #7c5cff; --accent-light: #a78bfa;
    --text: #ece9f7; --muted: #8f8aa8; --border: #322d4a;
  }
  * { box-sizing: border-box; }
  ...
  .avatar { width: 64px; height: 64px; border-radius: 50%; background: var(--panel2); flex-shrink: 0; object-fit: cover; }
  .name { font-size: 22px; font-weight: 700; }
  .sub { color: var(--muted); font-size: 13px; margin-top: 2px; }
  .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
  .card {
    background: var(--panel); border: 1px solid var(--border); border-radius: 14px;
    padding: 14px; grid-column: span 1;
  }
  .card.wide { grid-column: span 2; }
  .card-label { color: var(--muted); font-size: 12px; text-transform: uppercase; letter-spacing: .04em; }
  .card-value { font-size: 22px; font-weight: 700; margin-top: 4px; color: var(--accent-light); }
...
async function loadProfile() {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) {
      content.innerHTML = `<div class="error">${data.error || 'Could not load this profile.'}</div>`;
      return;
    }
    content.innerHTML = `
      <div class="header">
        ${data.avatar_url ? `<img class="avatar" src="${data.avatar_url}">` : '<div class="avatar"></div>'}
        <div>
          <div class="name">${data.display_name}</div>
          <div class="sub">${data.sessions_count} session${data.sessions_count === 1 ? '' : 's'}${data.most_used_channel ? ' · ' + data.most_used_channel : ''}</div>
        </div>
      </div>
      <div class="grid">
        <div class="card wide"><div class="card-label">Total VC Time</div><div class="card-value">${data.formatted.total}</div></div>
        <div class="card"><div class="card-label">Today</div><div class="card-value">${data.formatted.today}</div></div>
        <div class="card"><div class="card-label">This Week</div><div class="card-value">${data.formatted.week}</div></div>
        <div class="card"><div class="card-label">This Month</div><div class="card-value">${data.formatted.month}</div></div>
        <div class="card"><div class="card-label">Longest Session</div><div class="card-value">${data.formatted.longest_session}</div></div>
      </div>
    `;
  } catch (e) {
    content.innerHTML = '<div class="error">Something went wrong loading this profile.</div>';
  }
}
loadProfile();
</script>
...
def init_db():
    ...
    try:
        ...
        with _cursor() as cur:
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS doctor_temp_fixes (
                    id BIGSERIAL PRIMARY KEY,
                    fix_id TEXT NOT NULL UNIQUE,
                    component TEXT NOT NULL,
                    reason TEXT NOT NULL,
                    toggle_key TEXT NOT NULL,
                    previous_toggle_state BOOLEAN NOT NULL,
                    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
            ...
            """)
            # ---- Reservation redesign (Phase 1): new schema, old vc_reservations
            # table is left untouched below and still serves the existing
            # /vc-reserve etc. commands until Phase 2 replaces them. ----
            cur.execute("""
                CREATE TABLE IF NOT EXISTS reservations (
                    ...
                    reservation_type TEXT NOT NULL,           -- 'existing_vc' | 'private_vc'
                    requested_channel_id TEXT,
                    requested_channel_name TEXT,
                    assigned_channel_id TEXT,
                    assigned_channel_name TEXT,
                    start_time TIMESTAMPTZ NOT NULL,
                    end_time TIMESTAMPTZ NOT NULL,
                    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
            ...
            """)
            cur.execute("CREATE INDEX IF NOT EXISTS idx_reservations_requester ON reservations (requester_id, status)")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_reservations_status_time ON reservations (status, start_time)")
            cur.execute("""
            ...
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS temp_vc_pool (
                    channel_id TEXT PRIMARY KEY,
                    pool_label TEXT NOT NULL,
                    status TEXT NOT NULL DEFAULT 'available',   -- 'available' | 'in_use'
                    current_reservation_id BIGINT REFERENCES reservations(id) ON DELETE SET NULL
                )
            """)
            cur.execute("ALTER TABLE vc_sessions ADD COLUMN IF NOT EXISTS reservation_id BIGINT REFERENCES reservations(id) ON DELETE SET NULL")
            cur.execute("ALTER TABLE reservations ADD COLUMN IF NOT EXISTS approval_channel_id TEXT")
            cur.execute("ALTER TABLE reservations ADD COLUMN IF NOT EXISTS approval_message_id TEXT")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_vc_sessions_reservation ON vc_sessions (reservation_id) WHERE reservation_id IS NOT NULL")
        print("[db] PostgreSQL ready (vc_logs, vc_lock_channels, vc_lock_members, join_requests, bot_settings, birthdays, birthday_nudges, points, vc_sessions, vc_reservations, doctor_temp_fixes, reservations, reservation_members, temp_vc_pool).", flush=True)
        return True
...
TOGGLE_KEYS = {
    ...
    "reservations": "VC Reservation system (booking panel, approvals, temp VC pool)",
}
        ...
        try:
            with _cursor() as cur:
                ...
                cur.execute("""
                    ...
                    WHERE assigned_channel_id = %s AND status IN ('approved', 'active')
                    ORDER BY start_time DESC LIMIT 1
                """, (str(channel_id) if channel_id else None,))
                resv_row = cur.fetchone()
                reservation_id_for_session = resv_row["id"] if resv_row else None
                cur.execute("""
                    INSERT INTO vc_sessions
                        (user_id, user_name, channel_id, channel_name, started_at, start_is_estimated, reservation_id)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                """, (str(user_id), user_name, str(channel_id) if channel_id else None,
                      channel_name, at, start_is_estimated, reservation_id_for_session))
            return
        except (psycopg2.InterfaceError, psycopg2.OperationalError):
            if attempt == 0:
                continue
...
def fetch_sessions_for_period(period_start, period_end):
    ...
    for attempt in range(2):
        try:
            ...
            record_error("fetch_sessions_for_period", RuntimeError("failed after retry"))
            return []
        except Exception as e:
...
async def reconcile_vc_sessions(send_report: bool = True):
    """
    Smart Recovery: compares DB session state against real Discord presence
    and safely repairs mismatches. Runs once per bot connect/reconnect
    (alongside the existing, unchanged reconcile_stale_sessions() which
    keeps vc_logs itself coherent), and can also be triggered on demand via
    /run-smart-recovery.

    This is deliberately built ON TOP of the existing hybrid vc_sessions
    system rather than a second tracker — the actual comparison is pure
    dict math in vc_stats.classify_vc_state/compute_reconciliation_actions
    (unit tested in isolation); this function's only job is gathering the
    two real-world states, applying the repair, detecting one genuine
    data-integrity edge case (CONFLICT — see below), and reporting what it
    found so admins can see recovery actions instead of them happening
    silently.

    Categories (see vc_stats.classify_vc_state for the exact definitions):
      MATCH / MISSING / MISMATCH / NEW — every user_id in either the DB's
      open sessions or Discord's actual voice channels gets classified as
      exactly one of these and repaired accordingly.
      CONFLICT — a separate, narrower check: if the DB somehow has more
      than one open session for the same user (should be prevented by the
      partial unique index, but this checks for it directly rather than
      assuming the invariant always holds). This is flagged for review,
      NOT auto-resolved — Smart Recovery repairs state mismatches, but a
      duplicate-open-session anomaly is a correctness question worth a
      human look rather than a guess.

    Returns the report dict, so /run-smart-recovery can show the same
    thing on demand. Only actually DMs admins when send_report=True AND
    there was something to report — matching the "don't nudge people for
    no reason" lesson from the birthday-spam fix earlier.
    """
    if not _pool:
        return None
    guild = bot.get_guild(GUILD_ID)
    ...
    # ---- CONFLICT check: more than one open session for the same user ----
    with _cursor() as cur:
        cur.execute("""
            SELECT user_id, COUNT(*) AS n FROM vc_sessions
            WHERE ended_at IS NULL GROUP BY user_id HAVING COUNT(*) > 1
        """)
        conflicted_user_ids = {r["user_id"] for r in cur.fetchall()}

    with _cursor() as cur:
        cur.execute("SELECT user_id, channel_id FROM vc_sessions WHERE ended_at IS NULL")
        # If a user has a genuine conflict, exclude them from the normal
        # classify/repair pass entirely — acting on ambiguous state would be
        # guessing, not recovering. They still show up in the report.
        open_sessions = {
            r["user_id"]: r["channel_id"] for r in cur.fetchall() if r["user_id"] not in conflicted_user_ids
        }
    ...
    records = vs.classify_vc_state(open_sessions, currently_present)
    ...
    actionable = [r for r in records if r["category"] != "match"]

    def channel_name_for(channel_id):
        if channel_id is None:
            return None
        ch = guild.get_channel(int(channel_id)) if str(channel_id).isdigit() else None
        return ch.name if ch else str(channel_id)

    report = {
        "match_count": sum(1 for r in records if r["category"] == "match"),
        "actionable": [
            {**r, "db_channel_name": channel_name_for(r["db_channel"]), "discord_channel_name": channel_name_for(r["discord_channel"])}
            for r in actionable
        ],
        "conflicts": list(conflicted_user_ids),
    }

    if actionable or conflicted_user_ids:
        print(
            f"[smart_recovery] {report['match_count']} match(es), "
            f"{sum(1 for r in actionable if r['category'] == 'missing')} missing, "
            f"{sum(1 for r in actionable if r['category'] == 'mismatch')} mismatch, "
            f"{sum(1 for r in actionable if r['category'] == 'new')} new, "
            f"{len(conflicted_user_ids)} conflict(s) needing review.",
            flush=True,
        )
        if send_report:
            await send_smart_recovery_report(guild, report)

    return report


async def send_smart_recovery_report(guild: discord.Guild, report: dict):
    """DMs every ANNOUNCE_ADMIN_IDS admin a plain-text Smart Recovery report,
    formatted per user like the plan's example. Only called when there's
    something non-trivial to show."""
    lines = ["\U0001F6E1\uFE0F **SMART RECOVERY**", ""]
    category_labels = {
        "missing": ("Stale session closed", "SESSION_MISSING_ON_DISCORD"),
        "mismatch": ("Channel reconciled", "STATE_MISMATCH"),
        "new": ("Session opened", "PRESENT_WITH_NO_SESSION"),
    }
    for r in report["actionable"]:
        member = guild.get_member(int(r["user_id"]))
        name = member.display_name if member else r["user_id"]
        action, reason = category_labels[r["category"]]
        lines.append(f"**User:** {name}")
        lines.append(f"Database VC: {r['db_channel_name'] or '—'}")
        lines.append(f"Discord VC: {r['discord_channel_name'] or '—'}")
        lines.append(f"Action: {action}")
        lines.append(f"Reason: {reason}")
        lines.append("")

    if report["conflicts"]:
        lines.append("\u26A0\uFE0F **CONFLICTS NEEDING REVIEW** (not auto-fixed):")
        for uid in report["conflicts"]:
            member = guild.get_member(int(uid))
            name = member.display_name if member else uid
            lines.append(f"\u2022 {name} — multiple open sessions in vc_sessions. Check manually before /reset-vc-stats.")
        lines.append("")
    ...
    for admin_id in ANNOUNCE_ADMIN_IDS:
        try:
            admin = bot.get_user(int(admin_id)) or await bot.fetch_user(int(admin_id))
            for chunk_start in range(0, len(text), 1900):  # stay under Discord's 2000-char DM limit
                await admin.send(text[chunk_start:chunk_start + 1900])
        except (discord.Forbidden, discord.NotFound, ValueError):
            pass


# ============================================================
# DOCTOR MODE — read-only diagnosis, never a blind code-editor
# ============================================================
# Doctor Mode only ever INSPECTS and REPORTS by default. The one exception —
# temporary fixes — are proposed with full metadata (Fix ID, reason,
# component, expiration) and require an explicit admin button click to
# actually apply. Nothing here edits source code, ever, automatically.

DOCTOR_TEMP_FIX_DURATION_HOURS = 1.0
SUSPICIOUS_SESSION_HOURS = 48  # a completed session longer than this gets flagged for review, not acted on
STALE_OPEN_SESSION_HOURS = 24  # an open session older than this (that Smart Recovery hasn't already resolved) is worth a diagnostic flag


async def run_doctor_diagnosis() -> list:
    """
    Read-only health scan across the areas the plan asks for. Returns a list
    of issue dicts: {component, file, function, problem, severity, diagnosis,
    proposed_action}. Nothing here modifies anything — this function only
    looks and reports, matching "the diagnostic log must be generated BEFORE
    any repair/change."
    """
    issues = []
    ...
    if bot is None or not bot.is_ready():
        issues.append({
            "component": "Discord connection", "file": "app.py", "function": "on_ready",
            "problem": "Bot is not currently connected/ready.",
            "severity": "CRITICAL", "diagnosis": "Gateway connection is down or still starting up.",
            "proposed_action": "Wait for reconnect; check Render logs for gateway errors.",
        })
    elif bot.latency and bot.latency > 2.0:
        issues.append({
            "component": "Discord connection", "file": "app.py", "function": "on_ready",
            "problem": f"High gateway latency: {bot.latency:.1f}s.",
            "severity": "LOW", "diagnosis": "Likely network conditions or Discord-side slowness, not a code issue.",
            "proposed_action": "Monitor — no action needed unless it persists.",
        })
    ...
    try:
        ...
        issues.append({
            "component": "Database connection", "file": "app.py", "function": "_cursor",
            "problem": f"A trivial SELECT 1 failed: {e}",
            "severity": "CRITICAL", "diagnosis": "Postgres is unreachable or the connection pool is exhausted.",
            "proposed_action": "Check DATABASE_URL and Render Postgres status.",
        })
        return issues  # nothing else below can be trusted without a working DB

    # ---- Duplicate open sessions (CONFLICT) ----
    with _cursor() as cur:
        cur.execute("SELECT user_id, COUNT(*) AS n FROM vc_sessions WHERE ended_at IS NULL GROUP BY user_id HAVING COUNT(*) > 1")
        dupes = cur.fetchall()
    if dupes:
        issues.append({
            "component": "Session Recovery", "file": "vc_stats.py", "function": "classify_vc_state",
            "problem": f"{len(dupes)} user(s) have more than one OPEN vc_sessions row simultaneously.",
            "severity": "HIGH", "diagnosis": "Should be prevented by the partial unique index — indicates a race or a direct write that bypassed open_or_move_session().",
            "proposed_action": "Review manually via /run-smart-recovery's conflict list before running /reset-vc-stats.",
        })

    # ---- Invalid timestamps ----
    with _cursor() as cur:
        cur.execute("SELECT COUNT(*) AS n FROM vc_sessions WHERE ended_at IS NOT NULL AND ended_at < started_at")
        bad_order = cur.fetchone()["n"]
        cur.execute("SELECT COUNT(*) AS n FROM vc_sessions WHERE duration_seconds < 0")
        bad_duration = cur.fetchone()["n"]
    if bad_order or bad_duration:
        issues.append({
            "component": "Session Consistency", "file": "app.py", "function": "open_or_move_session / close_session",
            "problem": f"{bad_order} session(s) end before they start; {bad_duration} have negative duration.",
            "severity": "HIGH", "diagnosis": "Likely a clock/timezone bug or manual data edit.",
            "proposed_action": "Inspect the specific rows via /inspect-vc-logs for the affected user(s) before correcting.",
        })

    # ---- Overlapping sessions for the same user (should never happen — one open session at a time by design) ----
    with _cursor() as cur:
        cur.execute("""
            SELECT a.user_id, COUNT(*) AS n
            FROM vc_sessions a JOIN vc_sessions b
    ...
    if overlaps:
        issues.append({
            "component": "Session Consistency", "file": "app.py", "function": "open_or_move_session",
            "problem": f"{len(overlaps)} user(s) have two CLOSED sessions whose time ranges overlap.",
            "severity": "MEDIUM", "diagnosis": "Shouldn't be possible under normal JOIN/MOVE/LEAVE handling — worth reviewing the affected raw vc_logs.",
            "proposed_action": "Use /inspect-vc-logs on the affected user(s).",
        })

    # ---- Suspiciously long completed sessions ----
    with _cursor() as cur:
        cur.execute("SELECT COUNT(*) AS n FROM vc_sessions WHERE ended_at IS NOT NULL AND duration_seconds > %s", (SUSPICIOUS_SESSION_HOURS * 3600,))
        long_sessions = cur.fetchone()["n"]
    if long_sessions:
        issues.append({
            "component": "Statistics Consistency", "file": "vc_stats.py", "function": "reconstruct_sessions_with_ids",
            "problem": f"{long_sessions} completed session(s) longer than {SUSPICIOUS_SESSION_HOURS}h.",
            "severity": "LOW", "diagnosis": "Could be genuine (someone really was in VC that long) or old stale data from before Smart Recovery existed — same class of issue found during the original audit.",
            "proposed_action": "Review with /inspect-vc-logs; delete a bad row with /delete-vc-log if confirmed stale.",
        })

    # ---- Stale-looking open sessions (informational — Smart Recovery already handles real ones on reconnect) ----
    with _cursor() as cur:
        cur.execute("SELECT COUNT(*) AS n FROM vc_sessions WHERE ended_at IS NULL AND started_at < %s",
                     (datetime.now(timezone.utc) - timedelta(hours=STALE_OPEN_SESSION_HOURS),))
        old_open = cur.fetchone()["n"]
    if old_open:
        issues.append({
            "component": "Session Recovery", "file": "app.py", "function": "reconcile_vc_sessions",
            "problem": f"{old_open} session(s) have been open for over {STALE_OPEN_SESSION_HOURS}h.",
            "severity": "LOW", "diagnosis": "May be genuine long calls, or sessions Smart Recovery hasn't had a chance to re-check yet.",
            "proposed_action": "Run /run-smart-recovery now to re-check these against live presence.",
        })

    # ---- Runtime exceptions (from the in-memory buffer) ----
    recent_cutoff = datetime.now(timezone.utc) - timedelta(hours=1)
    recent = [e for e in recent_errors if e["time"] >= recent_cutoff]
    if recent:
        by_component = defaultdict(int)
        for e in recent:
            by_component[e["component"]] += 1
        breakdown = ", ".join(f"{comp} x{n}" for comp, n in by_component.items())
        issues.append({
            "component": "Runtime Exceptions", "file": "app.py", "function": "(various)",
            "problem": f"{len(recent)} error(s) logged in the last hour: {breakdown}.",
            "severity": "MEDIUM" if len(recent) < 5 else "HIGH",
            "diagnosis": "Repeated failures in the same component may indicate a persistent issue (e.g. a flaky DB connection) rather than a one-off blip.",
            "proposed_action": "If one component dominates, consider a temporary fix to disable it while investigating (see /doctor-mode's proposal, if offered).",
        })

    # ---- Existing test suite ----
    try:
        result = subprocess.run(
            [sys.executable, "-m", "pytest", "test_vc_stats.py", "-q"],
            capture_output=True, text=True, timeout=30, cwd=os.path.dirname(os.path.abspath(__file__)),
        )
        if result.returncode != 0:
            issues.append({
                "component": "Test Suite", "file": "test_vc_stats.py", "function": "(pytest)",
                "problem": "The vc_stats test suite is NOT passing.",
                "severity": "CRITICAL",
                "diagnosis": (result.stdout[-500:] or result.stderr[-500:] or "See logs.").strip(),
                "proposed_action": "Do not deploy further VC tracking changes until this is fixed.",
            })
    except Exception as e:
        issues.append({
            "component": "Test Suite", "file": "test_vc_stats.py", "function": "(pytest)",
            "problem": f"Could not run the test suite: {e}",
            "severity": "LOW", "diagnosis": "pytest may not be installed in this environment, or the file is missing.",
            "proposed_action": "Verify pytest is available and test_vc_stats.py is deployed alongside app.py.",
        })

    return issues


def format_doctor_report(issues: list) -> str:
    if not issues:
        return "\U0001FA7A **VC DOCTOR REPORT**\n\nNo issues found. Everything checked out clean."

    lines = ["\U0001FA7A **VC DOCTOR REPORT**", ""]
    for i, issue in enumerate(issues, 1):
        lines.append(f"\u26A0\uFE0F ISSUE #{i}")
        ...
        lines.append(f"Diagnosis: {issue['diagnosis']}")
        lines.append(f"Proposed action: {issue['proposed_action']}")
        if issue["severity"] == "CRITICAL":
            lines.append(
                "\U0001F527 **This needs a PERMANENT code fix from an admin.** Doctor Mode never "
                "edits source code automatically — review and fix this manually."
            )
        lines.append("")
    lines.append("\u26A0\uFE0F NO CODE CHANGED YET")
    return "\n".join(lines)


async def send_doctor_report_to_admins(report_text: str, view: discord.ui.View = None):
    """DMs the full Doctor Mode report (and any fix-proposal buttons) to every
    admin in ANNOUNCE_ADMIN_IDS — not just whoever ran the command. If
    multiple admins get the same fix-proposal buttons, only the first click
    actually applies it (apply_temp_fix is idempotent); everyone else just
    sees it was already handled."""
    for admin_id in ANNOUNCE_ADMIN_IDS:
        try:
            admin = bot.get_user(int(admin_id)) or await bot.fetch_user(int(admin_id))
            chunks = [report_text[i:i + 1900] for i in range(0, len(report_text), 1900)]
            for i, chunk in enumerate(chunks):
                # Buttons only need to appear once — attach them to the last chunk
                await admin.send(chunk, view=view if i == len(chunks) - 1 else None)
        except (discord.Forbidden, discord.NotFound, ValueError):
            pass


# ---------- Temporary fixes (proposed, never auto-applied without a click) ----------

def propose_temp_fix(component: str, reason: str, toggle_key: str) -> str:
    """Records a PROPOSED (not yet active) temporary fix and returns its Fix ID.
    Applying it is a separate, explicit step (see DoctorFixProposalView)."""
    fix_id = f"DOC-{int(datetime.now(timezone.utc).timestamp())}"
    previous_state = get_toggle(toggle_key)
    expires_at = datetime.now(timezone.utc) + timedelta(hours=DOCTOR_TEMP_FIX_DURATION_HOURS)
    with _cursor() as cur:
        cur.execute("""
            INSERT INTO doctor_temp_fixes (fix_id, component, reason, toggle_key, previous_toggle_state, expires_at, status)
            VALUES (%s, %s, %s, %s, %s, %s, 'proposed')
        """, (fix_id, component, reason, toggle_key, previous_state, expires_at))
    return fix_id


def apply_temp_fix(fix_id: str, admin_name: str) -> bool:
    """Actually flips the toggle off and marks the fix active. Called ONLY
    from the admin's explicit button click — never automatically."""
    with _cursor() as cur:
        cur.execute("SELECT toggle_key FROM doctor_temp_fixes WHERE fix_id = %s AND status = 'proposed'", (fix_id,))
        row = cur.fetchone()
        if not row:
            return False
        set_toggle(row["toggle_key"], False)
        cur.execute(
            "UPDATE doctor_temp_fixes SET status = 'active', applied_by = %s WHERE fix_id = %s",
            (admin_name, fix_id),
        )
    return True


def dismiss_temp_fix(fix_id: str) -> bool:
    with _cursor() as cur:
        cur.execute("UPDATE doctor_temp_fixes SET status = 'dismissed' WHERE fix_id = %s AND status = 'proposed' RETURNING id", (fix_id,))
        return cur.fetchone() is not None


async def expire_due_temp_fixes():
    """Reverts any ACTIVE temp fix past its expiration back to its previous
    toggle state. Reverting is inherently safe (restoring known-good prior
    state), so this part runs automatically on a timer — unlike applying a
    fix in the first place, which always needs an explicit admin click."""
    if not _pool:
        return
    with _cursor() as cur:
        cur.execute("""
            SELECT fix_id, toggle_key, previous_toggle_state FROM doctor_temp_fixes
            WHERE status = 'active' AND expires_at < NOW()
        """)
        due = cur.fetchall()
    for row in due:
        set_toggle(row["toggle_key"], row["previous_toggle_state"])
        with _cursor() as cur:
            cur.execute("UPDATE doctor_temp_fixes SET status = 'expired' WHERE fix_id = %s", (row["fix_id"],))
        print(f"[doctor] Temp fix {row['fix_id']} expired — reverted {row['toggle_key']} to {row['previous_toggle_state']}.", flush=True)


async def temp_fix_expiry_loop():
    await bot.wait_until_ready()
    while not bot.is_closed():
        try:
            await expire_due_temp_fixes()
        except Exception as e:
            print(f"[doctor] Temp fix expiry check failed (non-fatal): {e}", flush=True)
        await asyncio.sleep(600)  # check every 10 minutes


async def reservation_expiry_loop():
    """Periodically expires stale PENDING reservations (editing their
    approval message so it's no longer actionable), AND force-completes any
    APPROVED/ACTIVE reservation that's run past its end_time + grace period
    — the 'end-time safety mechanism' so an abandoned reservation with
    people still inside can't stay active forever."""
    await bot.wait_until_ready()
    while not bot.is_closed():
        try:
            expired_ids = expire_stale_pending_reservations()
            for reservation_id in expired_ids:
                reservation = get_reservation(reservation_id)
                if not reservation or not reservation.get("approval_message_id"):
                    continue
                channel = bot.get_channel(int(reservation["approval_channel_id"]))
                if channel is None:
                    continue
                try:
                    msg = await channel.fetch_message(int(reservation["approval_message_id"]))
                    members = list_reservation_members(reservation_id)
                    embed = _build_reservation_embed(reservation, members, guild=channel.guild)
                    embed.color = discord.Color.greyple()
                    embed.add_field(name="\u23F0 Decision", value="Expired \u2014 no admin action taken in time.", inline=False)
                    await msg.edit(embed=embed, view=None)
                except (discord.NotFound, discord.Forbidden):
                    pass
                await _dm_requester(reservation_id, "Your VC reservation request expired before an admin reviewed it. Feel free to submit a new one.")
            if expired_ids:
                print(f"[reservations] Expired {len(expired_ids)} stale pending reservation(s).", flush=True)

            now = datetime.now(timezone.utc)
            overdue = list_reservations(status="approved", limit=200) + list_reservations(status="active", limit=200)
            for reservation in overdue:
                if not rl.is_reservation_overdue(reservation["end_time"], now):
                    continue
                if update_reservation_status(reservation["id"], "completed"):
                    print(f"[reservations] Force-completed overdue reservation #{reservation['id']}.", flush=True)
                    if reservation.get("assigned_channel_id"):
                        guild = bot.get_guild(GUILD_ID)
                        with _cursor() as cur:
                            cur.execute("SELECT 1 FROM temp_vc_pool WHERE channel_id = %s", (reservation["assigned_channel_id"],))
                            is_pool_channel = cur.fetchone() is not None
                        if guild and is_pool_channel:
                            await recycle_temp_vc(guild, reservation["assigned_channel_id"])
                    await _dm_requester(reservation["id"], "Your VC reservation's time has ended and it's now marked complete.")
        except Exception as e:
            print(f"[reservations] Expiry sweep failed (non-fatal): {e}", flush=True)
        await asyncio.sleep(1800)  # check every 30 minutes


class DoctorFixProposalView(discord.ui.View):
    """DMed to admins only when Doctor Mode finds a repeated-error pattern
    worth a temporary workaround. Nothing is applied until a human clicks."""

    def __init__(self, fix_id: str, component: str, toggle_key: str):
        super().__init__(timeout=3600)
        self.fix_id = fix_id
        self.component = component
        self.toggle_key = toggle_key

    @discord.ui.button(label="Apply temporary fix", style=discord.ButtonStyle.danger, emoji="\U0001F691")
    async def apply(self, interaction: discord.Interaction, button: discord.ui.Button):
        ok = apply_temp_fix(self.fix_id, interaction.user.display_name)
        for child in self.children:
            child.disabled = True
        if ok:
            await interaction.response.edit_message(
                content=(
                    f"\U0001F691 **TEMPORARY FIX ACTIVE**\n\nFix ID: {self.fix_id}\nComponent: {self.component}\n"
                    f"Permanent source code changed: \u274C NO\n\n"
                    f"Auto-reverts in {DOCTOR_TEMP_FIX_DURATION_HOURS:.0f}h. Admin action required: fix the underlying cause."
                ),
                view=self,
            )
        else:
            await interaction.response.edit_message(content="Couldn't apply — this fix may have already been actioned or expired.", view=self)

    @discord.ui.button(label="Dismiss", style=discord.ButtonStyle.secondary)
    async def dismiss(self, interaction: discord.Interaction, button: discord.ui.Button):
        dismiss_temp_fix(self.fix_id)
        for child in self.children:
            child.disabled = True
        await interaction.response.edit_message(content="Dismissed — no change made.", view=self)
...
# ============================================================
# VC RESERVATIONS — REDESIGN (Phase 1: schema, state machine, migration)
# ============================================================
# Nothing in this section is wired into Discord yet (Phase 2/3). The old
# vc_reservations table and its functions/commands above are untouched and
# keep working exactly as before — this is purely additive.

RESERVATION_SPAM_CONFIG_KEYS = {
    "reservation_spam_protection_enabled": ("spam_protection_enabled", bool, True),
    "reservation_cooldown_minutes": ("cooldown_minutes", float, 30),
    "reservation_max_pending": ("max_pending", int, 1),
    "reservation_max_per_day": ("max_per_day", int, 3),
    "reservation_max_active": ("max_active", int, 1),
    "reservation_duplicate_protection_enabled": ("duplicate_protection_enabled", bool, True),
    "reservation_auto_expire_enabled": ("auto_expire_enabled", bool, True),
    "reservation_expire_after_hours": ("expire_after_hours", float, 24),
}


def get_reservation_spam_config() -> dict:
    config = {}
    with _cursor() as cur:
        cur.execute("SELECT key, value FROM bot_settings WHERE key = ANY(%s)", (list(RESERVATION_SPAM_CONFIG_KEYS.keys()),))
        stored = {r["key"]: r["value"] for r in cur.fetchall()}
    for db_key, (config_key, caster, default) in RESERVATION_SPAM_CONFIG_KEYS.items():
        raw = stored.get(db_key)
        if raw is None:
            config[config_key] = default
        elif caster is bool:
            config[config_key] = raw == "true"
        else:
            config[config_key] = caster(raw)
    return config


def set_reservation_spam_config_value(config_key: str, value):
    db_key = next((k for k, (ck, _, _) in RESERVATION_SPAM_CONFIG_KEYS.items() if ck == config_key), None)
    if db_key is None:
        raise ValueError(f"Unknown reservation spam config key: {config_key}")
...
def migrate_legacy_reservations():
    """
    One-time migration: copies the OLD vc_reservations rows (simple,
    approval-free, existing-VC-only bookings) into the new reservations +
    reservation_members schema, for historical continuity. The OLD table
    and its still-working commands are left completely untouched — this
    only ADDS corresponding rows to the new tables, never deletes or
    modifies the old ones. Gated by a bot_settings flag so it only ever
    runs once, no matter how many times the app redeploys afterward.
    """
    with _cursor() as cur:
        cur.execute("SELECT value FROM bot_settings WHERE key = 'legacy_reservations_migrated'")
        if cur.fetchone():
            print("[reservations] Legacy migration already completed previously — skipping.", flush=True)
            return

    with _cursor() as cur:
        cur.execute("""
            SELECT id, user_id, user_name, channel_id, channel_name, start_time, end_time, status, created_at
            FROM vc_reservations ORDER BY id ASC
        """)
        legacy_rows = [dict(r) for r in cur.fetchall()]

    if not legacy_rows:
        print("[reservations] No legacy vc_reservations rows to migrate.", flush=True)
    else:
        # Old system had no approval workflow — an 'active' legacy row was
        # already effectively granted, so it maps to 'approved' (not
        # re-litigated as newly pending). It also had no lock concept, so
        # migrated rows come in 'unlocked'.
        status_map = {"active": "approved", "cancelled": "cancelled"}
        migrated = 0
        with _cursor() as cur:
            for row in legacy_rows:
                new_status = status_map.get(row["status"], "approved")
                cur.execute("""
                    INSERT INTO reservations
                        (guild_id, requester_id, requester_name, reservation_name, reservation_type,
                         requested_channel_id, requested_channel_name, assigned_channel_id, assigned_channel_name,
                         start_time, end_time, created_at, status, lock_state, reason)
                    VALUES (%s, %s, %s, %s, 'existing_vc', %s, %s, %s, %s, %s, %s, %s, %s, 'unlocked', %s)
                    RETURNING id
                """, (
                    str(GUILD_ID), row["user_id"], row["user_name"], row["channel_name"],
                    row["channel_id"], row["channel_name"], row["channel_id"], row["channel_name"],
                    row["start_time"], row["end_time"], row["created_at"], new_status,
                    "Migrated from the legacy reservation system",
                ))
                new_id = cur.fetchone()["id"]
                cur.execute(
                    "INSERT INTO reservation_members (reservation_id, user_id, user_name) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING",
                    (new_id, row["user_id"], row["user_name"]),
                )
                migrated += 1
        print(f"[reservations] Migrated {migrated} legacy reservation(s) into the new schema.", flush=True)

    with _cursor() as cur:
        cur.execute(
            "INSERT INTO bot_settings (key, value) VALUES ('legacy_reservations_migrated', %s) "
            "ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",
            (datetime.now(timezone.utc).isoformat(),),
        )


def _reservation_time_ranges_for_user(user_id, exclude_reservation_id=None):
    with _cursor() as cur:
        cur.execute("""
            SELECT start_time, end_time FROM reservations
            WHERE requester_id = %s AND status IN ('pending', 'approved', 'active')
            AND id != COALESCE(%s, -1)
        """, (str(user_id), exclude_reservation_id))
        return [(r["start_time"], r["end_time"]) for r in cur.fetchall()]


def create_reservation_request(guild_id, requester_id, requester_name, reservation_name,
                                reservation_type, requested_channel_id, requested_channel_name,
                                start_time, end_time, reason, member_ids_and_names=None, lock_state="locked"):
    """
    Validates against spam control (reservation_logic.evaluate_new_reservation_request)
    before writing anything. member_ids_and_names: list of (user_id, user_name)
    tuples for the allowed member list — the requester is always included
    automatically even if the caller forgot to add them.

    Returns (ok, result): result is the new reservation_id on success, or a
    human-readable reason string if blocked by spam control.
    """
    now = datetime.now(timezone.utc)
    today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
    with _cursor() as cur:
        cur.execute("SELECT COUNT(*) AS n FROM reservations WHERE requester_id = %s AND status = 'pending'", (str(requester_id),))
        pending_count = cur.fetchone()["n"]
        cur.execute("SELECT COUNT(*) AS n FROM reservations WHERE requester_id = %s AND status = 'active'", (str(requester_id),))
        active_count = cur.fetchone()["n"]
        cur.execute("SELECT COUNT(*) AS n FROM reservations WHERE requester_id = %s AND created_at >= %s", (str(requester_id), today_start))
        today_count = cur.fetchone()["n"]
        cur.execute("SELECT MAX(created_at) AS t FROM reservations WHERE requester_id = %s", (str(requester_id),))
        last_request_time = cur.fetchone()["t"]

    existing_ranges = _reservation_time_ranges_for_user(requester_id)
    config = get_reservation_spam_config()

    allowed, reason_blocked = rl.evaluate_new_reservation_request(
        pending_count=pending_count, active_count=active_count, reservations_today_count=today_count,
        last_request_time=last_request_time, now=now,
        requested_start=start_time, requested_end=end_time,
        existing_time_ranges=existing_ranges, config=config,
    )
    if not allowed:
        return False, reason_blocked

    members = dict(member_ids_and_names or [])
    members[str(requester_id)] = requester_name  # requester is always included

    with _cursor() as cur:
        cur.execute("""
            INSERT INTO reservations
                (guild_id, requester_id, requester_name, reservation_name, reservation_type,
                 requested_channel_id, requested_channel_name, start_time, end_time, reason, status, lock_state)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'pending', %s)
            RETURNING id
        """, (str(guild_id), str(requester_id), requester_name, reservation_name, reservation_type,
              str(requested_channel_id) if requested_channel_id else None, requested_channel_name,
              start_time, end_time, reason, lock_state))
        new_id = cur.fetchone()["id"]
...
def list_reservation_members(reservation_id):
    with _cursor() as cur:
        cur.execute("SELECT user_id, user_name FROM reservation_members WHERE reservation_id = %s", (reservation_id,))
        return [dict(r) for r in cur.fetchall()]


def list_reservations(status=None, requester_id=None, limit=25):
...
def update_reservation_status(reservation_id, new_status, **extra_fields) -> bool:
    """
    Validates the transition via reservation_logic.can_transition BEFORE
    writing anything — this is what makes double-clicks, stale buttons, or
    out-of-order admin actions safe no-ops instead of corrupting state, and
    is exactly the 'idempotent actions' requirement from the spec (clicking
    Approve twice can't create two VCs if the second click's transition —
    approved -> approved — is simply invalid and rejected here).
    """
    with _cursor() as cur:
        ...
        if not row:
            return False
        if not rl.can_transition(row["status"], new_status):
            return False
        ...
        cur.execute(f"UPDATE reservations SET {', '.join(set_clauses)} WHERE id = %s", params)
    return True


def register_temp_vc_pool_channel(channel_id, pool_label):
    """Admin-side setup (Phase 3 UI will call this): adds a Discord channel
    to the reusable temp VC pool."""
    with _cursor() as cur:
        cur.execute(
            "INSERT INTO temp_vc_pool (channel_id, pool_label, status) VALUES (%s, %s, 'available') "
            "ON CONFLICT (channel_id) DO UPDATE SET pool_label = EXCLUDED.pool_label",
            (str(channel_id), pool_label),
        )


def assign_temp_vc(reservation_id):
    """
    Picks an available pool channel and marks it in_use for this
    reservation, atomically — both the read and the write happen in one
    transaction so two reservations approved at nearly the same instant can
    never be assigned the same pool channel. Returns the picked pool row
    dict, or None if the entire pool is currently busy.
    """
    with _cursor() as cur:
        cur.execute("SELECT channel_id, pool_label, status FROM temp_vc_pool ORDER BY channel_id ASC")
        pool_entries = [dict(r) for r in cur.fetchall()]
        picked = rl.select_available_temp_vc(pool_entries)
        if picked is None:
            return None
        cur.execute(
            "UPDATE temp_vc_pool SET status = 'in_use', current_reservation_id = %s WHERE channel_id = %s",
            (reservation_id, picked["channel_id"]),
        )
    return picked


def release_temp_vc_to_pool(channel_id):
    """Returns a temp VC to the reusable pool — called when it empties out
    (Phase 3) or a reservation using it is cancelled/completed. Never
    deletes the Discord channel, only marks it available again."""
    with _cursor() as cur:
        cur.execute(
            "UPDATE temp_vc_pool SET status = 'available', current_reservation_id = NULL WHERE channel_id = %s",
            (str(channel_id),),
        )


def expire_stale_pending_reservations() -> list:
    """Auto-expires PENDING reservations older than the configured
    threshold, if reservation_auto_expire_enabled is on. Returns the list
    of reservation IDs that were actually expired (so the caller can also
    update their Discord approval messages) — the expiry check itself is
    pure (reservation_logic.is_pending_expired), this just applies it and
    writes the result."""
    config = get_reservation_spam_config()
    if not config.get("auto_expire_enabled", True):
        return []
    now = datetime.now(timezone.utc)
    with _cursor() as cur:
        cur.execute("SELECT id, created_at FROM reservations WHERE status = 'pending'")
        pending = [dict(r) for r in cur.fetchall()]
    expired_ids = []
    for row in pending:
        if rl.is_pending_expired(row["created_at"], now, config["expire_after_hours"]):
            if update_reservation_status(row["id"], "expired"):
...
def get_reservation_booking_channel_id() -> int:
    with _cursor() as cur:
        cur.execute("SELECT value FROM bot_settings WHERE key = 'reservation_booking_channel_id'")
        row = cur.fetchone()
        return int(row["value"]) if row else 0


def set_reservation_booking_channel_id(channel_id: int):
    with _cursor() as cur:
        cur.execute(
            "INSERT INTO bot_settings (key, value) VALUES ('reservation_booking_channel_id', %s) "
            "ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",
            (str(channel_id),),
        )


def get_reservation_approval_channel_id() -> int:
    with _cursor() as cur:
        cur.execute("SELECT value FROM bot_settings WHERE key = 'reservation_approval_channel_id'")
        row = cur.fetchone()
        return int(row["value"]) if row else 0


def set_reservation_approval_channel_id(channel_id: int):
    with _cursor() as cur:
        cur.execute(
            "INSERT INTO bot_settings (key, value) VALUES ('reservation_approval_channel_id', %s) "
            "ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",
            (str(channel_id),),
        )


def set_reservation_approval_message(reservation_id, channel_id, message_id):
    with _cursor() as cur:
        cur.execute(
            "UPDATE reservations SET approval_channel_id = %s, approval_message_id = %s WHERE id = %s",
            (str(channel_id), str(message_id), reservation_id),
        )


# ---------- Phase 2: booking flow (member) ----------

def _parse_booking_datetime_range(date_start_str: str, end_time_str: str):
    """Parses the modals' combined date+start field and separate end-time
    field into UTC start/end datetimes, applying TIMEZONE_OFFSET_HOURS.
    Raises ValueError with a human-readable message on anything invalid."""
    try:
        naive_start = rl.parse_reservation_datetime(date_start_str)
    except ValueError:
        raise ValueError("Start needs to be in the format YYYY-MM-DD HH:MM, e.g. 2026-09-20 19:30.")
    try:
        end_hour, end_minute = rl.parse_reservation_end_time(end_time_str)
    except ValueError:
        raise ValueError("End time needs to be in the format HH:MM, e.g. 21:30.")

    naive_end = naive_start.replace(hour=end_hour, minute=end_minute)
    if naive_end <= naive_start:
        raise ValueError("End time must be after the start time (same-day reservations only for now).")

    start_utc = naive_start.replace(tzinfo=timezone.utc) - timedelta(hours=TIMEZONE_OFFSET_HOURS)
    end_utc = naive_end.replace(tzinfo=timezone.utc) - timedelta(hours=TIMEZONE_OFFSET_HOURS)
    if start_utc < datetime.now(timezone.utc):
        raise ValueError("That start time is already in the past.")
    return start_utc, end_utc


async def _resolve_member_mentions(guild: discord.Guild, member_ids: list) -> list:
    """Turns parsed user IDs into (id, display_name) pairs, skipping any that
    aren't actual members of this guild."""
    resolved = []
    for uid in member_ids:
        member = guild.get_member(int(uid))
        if member:
            resolved.append((str(uid), member.display_name))
    return resolved


async def _submit_booking(interaction: discord.Interaction, *, reservation_type: str,
                           reservation_name: str, requested_channel_id, requested_channel_name,
                           start_utc, end_utc, members_text: str = None, resolved_members: list = None,
                           reason: str, lock_state: str = "locked"):
    """Shared submission path for both booking flows — validates via
    create_reservation_request (spam control included), and on success
    posts the approval panel and confirms back to the requester.

    Accepts members either as free text to parse (the Private VC modal's
    path, unchanged) or as already-resolved (id, name) pairs (the new
    Existing VC form's native UserSelect path — no text parsing needed)."""
    if resolved_members is None:
        member_ids = rl.parse_member_ids(members_text or "")
        resolved_members = await _resolve_member_mentions(interaction.guild, member_ids)

    ok, result = create_reservation_request(
        guild_id=interaction.guild.id,
        requester_id=interaction.user.id, requester_name=interaction.user.display_name,
        reservation_name=reservation_name, reservation_type=reservation_type,
        requested_channel_id=requested_channel_id, requested_channel_name=requested_channel_name,
        start_time=start_utc, end_time=end_utc, reason=reason or None,
        member_ids_and_names=resolved_members, lock_state=lock_state,
    )
    if not ok:
    ...
    await interaction.response.send_message(
        "\u2705 Request submitted \u2014 waiting on admin approval. You'll get a DM once it's decided.",
        ephemeral=True,
    )
    await post_reservation_approval_message(reservation_id)


class PrivateVCBookingModal(discord.ui.Modal, title="Reserve a Private VC"):
    vc_name = discord.ui.TextInput(label="VC Name", placeholder="Gaming", max_length=60)
    date_start = discord.ui.TextInput(label="Start (YYYY-MM-DD HH:MM)", placeholder="2026-09-20 19:30")
    end_time = discord.ui.TextInput(label="End time (HH:MM, same day)", placeholder="21:30", max_length=5)
    members = discord.ui.TextInput(label="Members (@mentions or IDs)", style=discord.TextStyle.paragraph, required=False)
    reason = discord.ui.TextInput(label="Reason (optional)", required=False, max_length=200)

    async def on_submit(self, interaction: discord.Interaction):
        try:
            start_utc, end_utc = _parse_booking_datetime_range(str(self.date_start), str(self.end_time))
        except ValueError as e:
            await interaction.response.send_message(f"\u274C {e}", ephemeral=True)
            return
        await _submit_booking(
            interaction, reservation_type="private_vc", reservation_name=str(self.vc_name),
            requested_channel_id=None, requested_channel_name=None,
            start_utc=start_utc, end_utc=end_utc, members_text=str(self.members), reason=str(self.reason),
        )


def _mark_selected_option(select: discord.ui.Select, chosen_value: str):
    """Discord only shows a Select's chosen option as its displayed label
    (instead of falling back to the placeholder) if that option is marked
    default=True in the component definition — this doesn't happen
    automatically just because the Python-side state was updated. Called
    from each Select's callback right after a pick, so the dropdown
    visually keeps showing what was chosen instead of resetting to its
    placeholder on every subsequent interaction with the form."""
    for option in select.options:
        option.default = (option.value == chosen_value)


class ExistingVCChannelSelect(discord.ui.Select):
    """Row 0 of the Existing VC form."""
    ...
    def __init__(self, parent_view):
        ...
        super().__init__(
            placeholder="\U0001F4E1 Voice Channel", row=0,
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if channel:
            self.parent_view.channel_id, self.parent_view.channel_name = channel.id, channel.name
        _mark_selected_option(self, self.values[0])
        await self.parent_view.refresh(interaction)


class ExistingVCDateSelect(discord.ui.Select):
    """Row 1 — a real date picker instead of typing YYYY-MM-DD."""

    def __init__(self, parent_view):
        today_local = (datetime.now(timezone.utc) + timedelta(hours=TIMEZONE_OFFSET_HOURS)).date()
        options = [
            discord.SelectOption(label=label, value=d.isoformat())
            for d, label in rl.generate_date_options(today_local, days_ahead=14)
        ]
        super().__init__(placeholder="\U0001F4C5 Date", row=1, options=options)
        self.parent_view = parent_view

    async def callback(self, interaction: discord.Interaction):
        self.parent_view.date_value = datetime.strptime(self.values[0], "%Y-%m-%d").date()
        _mark_selected_option(self, self.values[0])
        await self.parent_view.refresh(interaction)


class ExistingVCStartTimeSelect(discord.ui.Select):
    """Row 2 — 12-hour AM/PM start time, no typing."""

    def __init__(self, parent_view):
        options = [discord.SelectOption(label=label, value=value) for value, label in rl.generate_hourly_time_options()]
        super().__init__(placeholder="\U0001F7E2 Start time", row=2, options=options)
        self.parent_view = parent_view

    async def callback(self, interaction: discord.Interaction):
        self.parent_view.start_hour = int(self.values[0])
        _mark_selected_option(self, self.values[0])
        self.parent_view.rebuild_end_time_options()
        await self.parent_view.refresh(interaction)


class ExistingVCEndTimeSelect(discord.ui.Select):
    """Row 3 — 12-hour AM/PM end time, no typing."""

    def __init__(self, parent_view):
        options = [discord.SelectOption(label=label, value=value) for value, label in rl.generate_hourly_time_options()]
        super().__init__(placeholder="\U0001F534 End time", row=3, options=options)
        self.parent_view = parent_view

    async def callback(self, interaction: discord.Interaction):
        self.parent_view.end_hour = int(self.values[0])
        _mark_selected_option(self, self.values[0])
        await self.parent_view.refresh(interaction)


class ExistingVCMemberPickerView(discord.ui.View):
    """A real Discord member picker — search, scroll, multi-select, all
    native to the Discord client. Sent as a follow-up when 'Members' is
    tapped, since the main form already uses all 5 available rows."""

    def __init__(self, parent_view):
        super().__init__(timeout=300)
        self.parent_view = parent_view
        self.picker.placeholder = "Search and select the members you want to allow."
        self.picker.min_values = 0
        self.picker.max_values = 10

    @discord.ui.select(cls=discord.ui.UserSelect, placeholder="Search and select the members you want to allow.")
    async def picker(self, interaction: discord.Interaction, select: discord.ui.UserSelect):
        self.parent_view.selected_members = {
            str(m.id): m.display_name for m in select.values if not m.bot
        }
        names_joined = ", ".join(self.parent_view.selected_members.values())
        if not names_joined:
            names_joined = "(none yet \u2014 you\u2019ll still be included automatically)"
        await interaction.response.edit_message(content=f"Selected: {names_joined}", view=self)
        await self.parent_view.refresh_message_only()


class ExistingVCReasonModal(discord.ui.Modal, title="Add a reason"):
    reason = discord.ui.TextInput(label="Reason (optional)", required=False, max_length=200, placeholder="Scrim with friends...")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        self.parent_view.reason = str(self.reason)
        await interaction.response.defer()
        await self.parent_view.refresh_message_only()


class ExistingVCBookingFormView(discord.ui.View):
    """
    The full Existing-VC booking form as one live-updating message: VC,
    Date, Start, End as native dropdowns (rows 0-3), then a button row for
    Members (native UserSelect follow-up) / Reason (short modal) / Lock-
    Unlock toggle / Submit (row 4) — exactly Discord's 5-row ceiling.
    Replaces the old channel-select-then-typed-modal flow; the Private VC
    modal is untouched.
    """
    ...
    def __init__(self, requester: discord.Member):
        ...
        self.end_hour = None
        self.selected_members = {}  # user_id -> display_name, requester added automatically at submit
        self.reason = ""
        self.lock_state = "locked"
        self.message = None  # set after the first send, so modal/follow-up callbacks can edit it back

        self.add_item(ExistingVCChannelSelect(self))
        self.add_item(ExistingVCDateSelect(self))
        self.add_item(ExistingVCStartTimeSelect(self))
        self.end_select = ExistingVCEndTimeSelect(self)
        self.add_item(self.end_select)
        self._style_lock_button()

    def rebuild_end_time_options(self):
        """Called whenever Start time changes: restricts End time to only
        hours AFTER the chosen start, so an invalid combo can't even be
        picked — instead of only catching it at Submit. If the previously
        chosen end hour is no longer valid, it's cleared so the form
        correctly shows End as unset again until re-picked."""
        if self.start_hour is None:
            valid = rl.generate_hourly_time_options()
        else:
            valid = [(v, l) for v, l in rl.generate_hourly_time_options() if int(v) > self.start_hour]

        if not valid:
            self.end_select.options = [discord.SelectOption(label="No end times available same day", value="none")]
            self.end_select.disabled = True
            self.end_hour = None
            return

        self.end_select.disabled = False
        valid_values = {v for v, _ in valid}
        if self.end_hour is not None and str(self.end_hour) not in valid_values:
            self.end_hour = None  # previous pick no longer valid — reset so the form shows it as unset

        options = [discord.SelectOption(label=label, value=value) for value, label in valid]
        for opt in options:
            opt.default = (self.end_hour is not None and opt.value == str(self.end_hour))
        self.end_select.options = options

    def _style_lock_button(self):
        if self.lock_state == "locked":
            self.toggle_lock.label, self.toggle_lock.emoji, self.toggle_lock.style = "Lock", "\U0001F512", discord.ButtonStyle.success
        else:
            self.toggle_lock.label, self.toggle_lock.emoji, self.toggle_lock.style = "Unlock", "\U0001F513", discord.ButtonStyle.secondary

    def _summary_embed(self) -> discord.Embed:
        embed = discord.Embed(
            title="\U0001F50A Reserve an Existing VC",
            description="Book a voice channel in under 30 seconds.",
            color=discord.Color.blurple(),
        )
        embed.add_field(name="\U0001F4E1 Voice Channel", value=self.channel_name or "\u2014 not selected", inline=False)
        embed.add_field(name="\U0001F4C5 Date", value=self.date_value.strftime("%d %b %Y") if self.date_value else "\u2014 not selected", inline=True)
        start_label = dict(rl.generate_hourly_time_options()).get(str(self.start_hour), "\u2014") if self.start_hour is not None else "\u2014"
        end_label = dict(rl.generate_hourly_time_options()).get(str(self.end_hour), "\u2014") if self.end_hour is not None else "\u2014"
        embed.add_field(name="\U0001F7E2 Start / \U0001F534 End", value=f"{start_label} \u2192 {end_label}", inline=True)
        member_names = list(self.selected_members.values())
        embed.add_field(
            name="\U0001F465 Members",
            value=(", ".join(member_names) if member_names else "Just you so far") + f"\n*({self.requester.display_name} is always included)*",
            inline=False,
        )
        if self.reason:
            embed.add_field(name="\U0001F4DD Reason", value=self.reason, inline=False)
        mode_text = (
            "\U0001F512 **Lock** \u2014 only the selected members can join the VC during the reservation."
            if self.lock_state == "locked" else
            "\U0001F513 **Unlock** \u2014 anyone can join the VC during the reservation."
        )
        embed.add_field(name="\U0001F510 Access Mode", value=mode_text, inline=False)
        embed.set_footer(text="The bot checks for time conflicts before submitting your reservation.")
        return embed
    ...
    async def refresh_message_only(self):
        """For callbacks (modal submit, member picker) that already responded
        to their OWN interaction and need to update the original form message
        separately via the stored message reference."""
        if self.message:
            try:
                await self.message.edit(embed=self._summary_embed(), view=self)
            except discord.HTTPException:
                pass

    @discord.ui.button(label="Members", emoji="\U0001F465", style=discord.ButtonStyle.secondary, row=4)
    async def pick_members(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            "Search and select the members you want to allow.",
            view=ExistingVCMemberPickerView(self), ephemeral=True,
        )

    @discord.ui.button(label="Reason", emoji="\U0001F4DD", style=discord.ButtonStyle.secondary, row=4)
    async def add_reason(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(ExistingVCReasonModal(self))

    @discord.ui.button(label="Lock", emoji="\U0001F512", style=discord.ButtonStyle.success, row=4)
    async def toggle_lock(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.lock_state = "unlocked" if self.lock_state == "locked" else "locked"
        self._style_lock_button()
        await self.refresh(interaction)

    @discord.ui.button(label="Submit Reservation", emoji="\u2705", style=discord.ButtonStyle.primary, row=4)
    async def submit(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        naive_start = datetime.combine(self.date_value, datetime.min.time()).replace(hour=self.start_hour)
        naive_end = datetime.combine(self.date_value, datetime.min.time()).replace(hour=self.end_hour)
        if naive_end <= naive_start:
            await interaction.response.send_message("\u274C End time must be after the start time (same-day reservations only for now).", ephemeral=True)
            return
        start_utc = naive_start.replace(tzinfo=timezone.utc) - timedelta(hours=TIMEZONE_OFFSET_HOURS)
        end_utc = naive_end.replace(tzinfo=timezone.utc) - timedelta(hours=TIMEZONE_OFFSET_HOURS)
        if start_utc < datetime.now(timezone.utc):
            await interaction.response.send_message("\u274C That start time is already in the past.", ephemeral=True)
            return

        resolved_members = list(self.selected_members.items())
        await _submit_booking(
            interaction, reservation_type="existing_vc", reservation_name=self.channel_name,
            requested_channel_id=self.channel_id, requested_channel_name=self.channel_name,
            start_utc=start_utc, end_utc=end_utc, resolved_members=resolved_members,
            reason=self.reason, lock_state=self.lock_state,
        )


class ReservationBookingPanelView(discord.ui.View):
    """The persistent panel posted in the reservation booking channel."""

    def __init__(self):
        super().__init__(timeout=None)
        self.existing_vc.custom_id = "reservation_panel_existing_vc"
        self.private_vc.custom_id = "reservation_panel_private_vc"

    @discord.ui.button(label="Existing VC", emoji="\U0001F50A", style=discord.ButtonStyle.primary)
    async def existing_vc(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not get_toggle("reservations"):
            await interaction.response.send_message("Reservations are turned off right now.", ephemeral=True)
            return
        form = ExistingVCBookingFormView(interaction.user)
        await interaction.response.send_message(embed=form._summary_embed(), view=form, ephemeral=True)
        form.message = await interaction.original_response()

    @discord.ui.button(label="Private VC", emoji="\U0001F512", style=discord.ButtonStyle.secondary)
    async def private_vc(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not get_toggle("reservations"):
            await interaction.response.send_message("Reservations are turned off right now.", ephemeral=True)
            return
        await interaction.response.send_modal(PrivateVCBookingModal())


async def ensure_reservation_panel_posted():
    channel_id = get_reservation_booking_channel_id()
    if not channel_id:
        return
    channel = bot.get_channel(channel_id)
    if channel is None:
        print(f"[reservations] Could not find booking channel {channel_id}.", flush=True)
        return
    async for msg in channel.history(limit=50):
        if msg.author.id == bot.user.id and msg.embeds and msg.embeds[0].title == "\U0001F4C5 VC RESERVATION":
            return  # already posted
    embed = discord.Embed(
        title="\U0001F4C5 VC RESERVATION",
        description="\U0001F50A **Existing VC** \u2014 reserve one of the server's current voice channels\n\U0001F512 **Private VC** \u2014 request a private temporary VC",
        color=discord.Color.blurple(),
    )
    await channel.send(embed=embed, view=ReservationBookingPanelView())


# ---------- Phase 2: admin approval flow ----------

async def post_reservation_approval_message(reservation_id):
    approval_channel_id = get_reservation_approval_channel_id()
    if not approval_channel_id:
        print(f"[reservations] No approval channel configured — reservation #{reservation_id} has no approval message.", flush=True)
        return
    channel = bot.get_channel(approval_channel_id)
    if channel is None:
        print(f"[reservations] Could not find approval channel {approval_channel_id}.", flush=True)
        return
    ...
    embed = _build_reservation_embed(reservation, members, guild=channel.guild)
    view = (
        ReservationApprovalViewExisting(reservation_id)
        if reservation["reservation_type"] == "existing_vc"
        else ReservationApprovalViewPrivate(reservation_id)
    )
    msg = await channel.send(embed=embed, view=view)
    set_reservation_approval_message(reservation_id, channel.id, msg.id)


def _build_reservation_embed(reservation: dict, members: list, guild: discord.Guild) -> discord.Embed:
    type_label = "\U0001F50A Existing VC" if reservation["reservation_type"] == "existing_vc" else "\U0001F512 Private VC"
    start_local = reservation["start_time"] + timedelta(hours=TIMEZONE_OFFSET_HOURS)
    end_local = reservation["end_time"] + timedelta(hours=TIMEZONE_OFFSET_HOURS)
    member_lines = []
    for m in members:
        member = guild.get_member(int(m["user_id"])) if guild else None
        member_lines.append(f"\u2022 {member.mention if member else m['user_name']}")

    embed = discord.Embed(title="\U0001F514 VC RESERVATION REQUEST", color=discord.Color.orange())
    embed.add_field(name="\U0001F464 Requested by", value=f"<@{reservation['requester_id']}>", inline=False)
    embed.add_field(name="\U0001F4CC Requested type", value=type_label, inline=False)
    if reservation["reservation_type"] == "existing_vc":
        embed.add_field(name="\U0001F3AE Requested VC", value=reservation["requested_channel_name"] or "\u2014", inline=False)
    else:
        embed.add_field(name="\U0001F3AE Requested name", value=reservation["reservation_name"] or "\u2014", inline=False)
    embed.add_field(name="\U0001F4C5 Date", value=start_local.strftime("%d %B %Y"), inline=True)
    embed.add_field(name="\u23F0 Time", value=f"{start_local.strftime('%I:%M %p')} \u2192 {end_local.strftime('%I:%M %p')}", inline=True)
    embed.add_field(name="\U0001F465 Members", value="\n".join(member_lines) or "\u2014", inline=False)
    if reservation.get("reason"):
        embed.add_field(name="\U0001F4DD Reason", value=reservation["reason"], inline=False)
    embed.set_footer(text=f"Reservation #{reservation['id']}")
    return embed


async def _finalize_approval_message(interaction: discord.Interaction, reservation_id, outcome_text: str, color: discord.Color):
    reservation = get_reservation(reservation_id)
    members = list_reservation_members(reservation_id)
    embed = _build_reservation_embed(reservation, members, guild=interaction.guild)
    embed.color = color
    embed.add_field(name="\u2705 Decision", value=outcome_text, inline=False)
    await interaction.response.edit_message(embed=embed, view=None)


async def _dm_requester(reservation_id, text: str):
    reservation = get_reservation(reservation_id)
    if reservation is None:
        return
    try:
        user = bot.get_user(int(reservation["requester_id"])) or await bot.fetch_user(int(reservation["requester_id"]))
        await user.send(text)
    except (discord.Forbidden, discord.NotFound):
        pass


class ReservationApprovalViewExisting(discord.ui.View):
    """Shown for an EXISTING VC request — admin can grant it as requested,
    grant a private VC instead, or decline. Per the locked-in requirement:
    'Give Private VC' does NOT touch the originally-requested existing
    channel at all — it only assigns a pool channel instead."""

    def __init__(self, reservation_id):
        super().__init__(timeout=None)
        self.reservation_id = reservation_id
        self.give_existing.custom_id = f"resv_give_existing_{reservation_id}"
        self.give_private.custom_id = f"resv_give_private_{reservation_id}"
        self.decline.custom_id = f"resv_decline_{reservation_id}"

    @discord.ui.button(label="Give Existing VC", emoji="\U0001F50A", style=discord.ButtonStyle.success)
    async def give_existing(self, interaction: discord.Interaction, button: discord.ui.Button):
        reservation = get_reservation(self.reservation_id)
        if reservation is None:
            await interaction.response.send_message("Reservation not found.", ephemeral=True)
            return
        ok = update_reservation_status(
            self.reservation_id, "approved",
            assigned_channel_id=reservation["requested_channel_id"],
            assigned_channel_name=reservation["requested_channel_name"],
            approved_at=datetime.now(timezone.utc), approved_by=interaction.user.display_name,
        )
        if not ok:
            await interaction.response.send_message("Already actioned by someone else.", ephemeral=True)
            return
        await _finalize_approval_message(interaction, self.reservation_id, f"\U0001F50A Given existing VC \u2014 by {interaction.user.display_name}", discord.Color.green())
        await _dm_requester(self.reservation_id, f"\u2705 Your reservation for **{reservation['requested_channel_name']}** was approved!")
        await send_reservation_control_panel(self.reservation_id)

    @discord.ui.button(label="Give Private VC", emoji="\U0001F512", style=discord.ButtonStyle.primary)
    async def give_private(self, interaction: discord.Interaction, button: discord.ui.Button):
        await _handle_give_private(interaction, self.reservation_id)

    @discord.ui.button(label="Decline", emoji="\u274C", style=discord.ButtonStyle.danger)
    async def decline(self, interaction: discord.Interaction, button: discord.ui.Button):
        await _handle_decline(interaction, self.reservation_id)


class ReservationApprovalViewPrivate(discord.ui.View):
    """Shown for a PRIVATE VC request — just Approve/Decline, since the
    member specifically didn't ask for an existing channel."""

    def __init__(self, reservation_id):
        super().__init__(timeout=None)
        self.reservation_id = reservation_id
        self.approve.custom_id = f"resv_approve_private_{reservation_id}"
        self.decline.custom_id = f"resv_decline_{reservation_id}"

    @discord.ui.button(label="Approve", emoji="\U0001F512", style=discord.ButtonStyle.success)
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        await _handle_give_private(interaction, self.reservation_id)
...
async def _handle_give_private(interaction: discord.Interaction, reservation_id):
    picked = assign_temp_vc(reservation_id)
    if picked is None:
        await interaction.response.send_message("\u274C No temporary VCs available right now \u2014 the pool is fully in use.", ephemeral=True)
        return
    ok = update_reservation_status(
        reservation_id, "approved",
        assigned_channel_id=picked["channel_id"], assigned_channel_name=picked["pool_label"],
        approved_at=datetime.now(timezone.utc), approved_by=interaction.user.display_name,
    )
    if not ok:
        release_temp_vc_to_pool(picked["channel_id"])  # roll back — someone else already actioned this
        await interaction.response.send_message("Already actioned by someone else.", ephemeral=True)
        return
    await provision_temp_vc_for_reservation(interaction.guild, reservation_id)
    await _finalize_approval_message(interaction, reservation_id, f"\U0001F512 Given a private VC ({picked['pool_label']}) \u2014 by {interaction.user.display_name}", discord.Color.green())
    await _dm_requester(reservation_id, f"\u2705 Your reservation was approved \u2014 you've been given a private VC ({picked['pool_label']}).")
    await send_reservation_control_panel(reservation_id)


async def _handle_decline(interaction: discord.Interaction, reservation_id):
    ok = update_reservation_status(reservation_id, "declined", declined_at=datetime.now(timezone.utc))
    if not ok:
        await interaction.response.send_message("Already actioned by someone else.", ephemeral=True)
        return
    await _finalize_approval_message(interaction, reservation_id, f"\u274C Declined by {interaction.user.display_name}", discord.Color.red())
    await _dm_requester(reservation_id, "Your VC reservation request was declined.")


async def restore_reservation_views():
    """Re-registers persistent views on every bot connect/reconnect: the
    booking panel (fixed custom_ids), one approval view per still-PENDING
    reservation, and one control panel view per still-APPROVED/ACTIVE
    reservation (dynamic custom_ids encoding the reservation_id) — same
    pattern as restore_join_views() for the WhatsApp system."""
    bot.add_view(ReservationBookingPanelView())
    pending = list_reservations(status="pending", limit=200)
    for reservation in pending:
        view = (
            ReservationApprovalViewExisting(reservation["id"])
            if reservation["reservation_type"] == "existing_vc"
            else ReservationApprovalViewPrivate(reservation["id"])
        )
        bot.add_view(view)
    if pending:
        print(f"[reservations] Restored {len(pending)} pending approval view(s).", flush=True)

    active = list_reservations(status="approved", limit=200) + list_reservations(status="active", limit=200)
    for reservation in active:
        bot.add_view(ReservationControlView(reservation["id"], reservation["lock_state"]))
    if active:
        print(f"[reservations] Restored {len(active)} active control panel view(s).", flush=True)


# ============================================================
# Phase 3: real Discord-side temp VC provisioning + recycling
# ============================================================

TEMP_VC_POOL_HIDDEN_NAME_PREFIX = "\U0001F47B Temp VC"


async def provision_temp_vc_for_reservation(guild: discord.Guild, reservation_id):
    """
    Called right after a reservation is approved with an assigned pool
    channel. Actually configures Discord: renames the channel, and locks
    its permission overwrites down to ONLY the reservation's allowed
    members (+ the bot) — nobody else can even see it, closest Discord
    allows to full hiding (see the owner/Administrator caveat documented
    on register_temp_vc_pool_channel).
    """
    reservation = get_reservation(reservation_id)
    if reservation is None or not reservation.get("assigned_channel_id"):
        return
    channel = guild.get_channel(int(reservation["assigned_channel_id"]))
    if channel is None or not isinstance(channel, discord.VoiceChannel):
        print(f"[reservations] Could not find assigned temp VC channel for reservation #{reservation_id}.", flush=True)
        return

    members = list_reservation_members(reservation_id)
    new_name = rl.generate_temp_vc_name(reservation["requester_name"], reservation["reservation_name"])
    ...
    try:
        await channel.edit(name=new_name[:100], overwrites=overwrites, reason=f"VC reservation #{reservation_id}")
    except discord.Forbidden:
        print(f"[reservations] Missing permission to configure temp VC for reservation #{reservation_id}.", flush=True)


async def recycle_temp_vc(guild: discord.Guild, channel_id):
    """
    Called when a temp VC empties out. Never deletes the Discord channel —
    resets its permissions back to fully hidden, renames it back to its
    neutral pool label, and returns the pool slot to 'available'. Whatever
    reservation was using it gets marked completed.
    """
    with _cursor() as cur:
        cur.execute("SELECT channel_id, pool_label, current_reservation_id FROM temp_vc_pool WHERE channel_id = %s", (str(channel_id),))
    ...
    if channel is not None:
        try:
            await channel.edit(
                name=pool_row["pool_label"],
                overwrites={guild.default_role: discord.PermissionOverwrite(view_channel=False, connect=False),
                            guild.me: discord.PermissionOverwrite(view_channel=True, connect=True, manage_channels=True, move_members=True)},
                reason="Recycling temp VC back to the pool",
            )
        except discord.Forbidden:
            print(f"[reservations] Missing permission to recycle temp VC {channel_id}.", flush=True)

    if pool_row["current_reservation_id"]:
        update_reservation_status(pool_row["current_reservation_id"], "completed")
    release_temp_vc_to_pool(channel_id)
    print(f"[reservations] Recycled {pool_row['pool_label']} back to the pool.", flush=True)


async def activate_reservation_on_first_join(reservation):
    """PENDING/APPROVED -> ACTIVE happens the moment someone actually joins
    the assigned channel — matches the spec's BOOK -> APPROVAL -> PROVISION
    -> CONTROL -> USE flow, where 'USE' is what flips it to active."""
    update_reservation_status(reservation["id"], "active")


async def find_active_reservation_for_channel(channel_id):
    """Looks up whether a channel currently belongs to an approved/active
    reservation — either an existing VC that was granted, or an in-use
    pool channel. Returns the reservation dict or None."""
    with _cursor() as cur:
        cur.execute("""
...
def set_reservation_lock_state(reservation_id, new_state: str):
    with _cursor() as cur:
        cur.execute("UPDATE reservations SET lock_state = %s WHERE id = %s", (new_state, reservation_id))


class RemoveReservationMemberSelect(discord.ui.Select):
    def __init__(self, reservation_id, members: list, requester_id):
        removable = [m for m in members if m["user_id"] != str(requester_id)]  # requester can't remove themselves
        options = [discord.SelectOption(label=m["user_name"] or m["user_id"], value=m["user_id"]) for m in removable[:25]]
        super().__init__(
            placeholder="Select a member to remove...",
            options=options or [discord.SelectOption(label="No removable members", value="none")],
            disabled=not options,
        )
        self.reservation_id = reservation_id
    ...
    async def callback(self, interaction: discord.Interaction):
        with _cursor() as cur:
            cur.execute(
                "DELETE FROM reservation_members WHERE reservation_id = %s AND user_id = %s",
                (self.reservation_id, self.values[0]),
            )
        await interaction.response.edit_message(content="Removed.", view=None)


class AddReservationMembersModal(discord.ui.Modal, title="Add members"):
    members = discord.ui.TextInput(label="Members to add (@mentions or IDs)", style=discord.TextStyle.paragraph)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        member_ids = rl.parse_member_ids(str(self.members))
        resolved = await _resolve_member_mentions(interaction.guild, member_ids)
        with _cursor() as cur:
            for uid, uname in resolved:
                cur.execute(
                    "INSERT INTO reservation_members (reservation_id, user_id, user_name) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING",
                    (self.reservation_id, uid, uname),
                )
        current_members = list_reservation_members(self.reservation_id)
        names = ", ".join(m["user_name"] or m["user_id"] for m in current_members)
        await interaction.response.send_message(f"Updated member list: {names}", ephemeral=True)


class ReservationControlView(discord.ui.View):
    """DMed to the requester once their reservation is approved. Their
    controls only ever apply to THEIR OWN reservation — never general
    server admin permissions, per the spec's explicit requirement."""

    def __init__(self, reservation_id, lock_state: str):
        super().__init__(timeout=None)
        self.reservation_id = reservation_id
        self.manage_members.custom_id = f"resv_manage_members_{reservation_id}"
        self.toggle_lock.custom_id = f"resv_toggle_lock_{reservation_id}"
        self.end_reservation.custom_id = f"resv_end_{reservation_id}"
        if lock_state == "locked":
            self.toggle_lock.label, self.toggle_lock.emoji, self.toggle_lock.style = "Unlock", "\U0001F513", discord.ButtonStyle.success
        else:
            self.toggle_lock.label, self.toggle_lock.emoji, self.toggle_lock.style = "Lock", "\U0001F512", discord.ButtonStyle.secondary

    async def _verify_owner(self, interaction: discord.Interaction):
        reservation = get_reservation(self.reservation_id)
        if reservation is None or str(interaction.user.id) != reservation["requester_id"]:
            await interaction.response.send_message("This isn't your reservation.", ephemeral=True)
            return None
        return reservation

    @discord.ui.button(label="Manage Members", emoji="\U0001F465", style=discord.ButtonStyle.secondary)
    async def manage_members(self, interaction: discord.Interaction, button: discord.ui.Button):
        reservation = await self._verify_owner(interaction)
        if not reservation:
            return
        await interaction.response.send_modal(AddReservationMembersModal(self.reservation_id))
        members = list_reservation_members(self.reservation_id)
        if len(members) > 1:  # more than just the requester
            remove_view = discord.ui.View(timeout=120)
            remove_view.add_item(RemoveReservationMemberSelect(self.reservation_id, members, reservation["requester_id"]))
            await interaction.followup.send("Or remove someone:", view=remove_view, ephemeral=True)
    ...
    async def toggle_lock(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not reservation:
            return
        new_state = "unlocked" if reservation["lock_state"] == "locked" else "locked"
        set_reservation_lock_state(self.reservation_id, new_state)
        new_view = ReservationControlView(self.reservation_id, new_state)
        status_text = "\U0001F512 LOCKED (selected members only)" if new_state == "locked" else "\U0001F513 UNLOCKED (anyone can join)"
        await interaction.response.edit_message(content=f"Status: {status_text}", view=new_view)

    @discord.ui.button(label="End Reservation", emoji="\u274C", style=discord.ButtonStyle.danger)
    async def end_reservation(self, interaction: discord.Interaction, button: discord.ui.Button):
        reservation = await self._verify_owner(interaction)
        if not reservation:
            return
        ok = update_reservation_status(self.reservation_id, "cancelled")
        if not ok:
            await interaction.response.send_message("This reservation already ended.", ephemeral=True)
            return
        if reservation.get("assigned_channel_id"):
            with _cursor() as cur:
                cur.execute("SELECT status FROM temp_vc_pool WHERE channel_id = %s", (reservation["assigned_channel_id"],))
                pool_row = cur.fetchone()
            if pool_row and pool_row["status"] == "in_use":
                channel = interaction.guild.get_channel(int(reservation["assigned_channel_id"]))
                if channel and len(channel.members) == 0:
                    await recycle_temp_vc(interaction.guild, reservation["assigned_channel_id"])
        for child in self.children:
            child.disabled = True
        await interaction.response.edit_message(content="Reservation ended.", view=self)


async def send_reservation_control_panel(reservation_id):
    reservation = get_reservation(reservation_id)
    if reservation is None:
        return
    try:
        user = bot.get_user(int(reservation["requester_id"])) or await bot.fetch_user(int(reservation["requester_id"]))
        vc_label = reservation["assigned_channel_name"] or reservation["reservation_name"]
        status_text = "\U0001F512 LOCKED" if reservation["lock_state"] == "locked" else "\U0001F513 UNLOCKED"
        await user.send(
            f"\U0001F3AE **{vc_label}**\nYour reservation is approved.\n\nStatus: {status_text}",
            view=ReservationControlView(reservation_id, reservation["lock_state"]),
        )
    except (discord.Forbidden, discord.NotFound):
        pass
...
recent_errors = deque(maxlen=100)  # for Doctor Mode's runtime-exception check — see record_error()


def record_error(component: str, error: Exception):
    """Lightweight, additive error capture — Doctor Mode reads this to report
    genuine runtime exception counts instead of fabricating a claim. Call
    sites are existing broad except-blocks; this only appends, never changes
    behavior."""
        ...
        try:
            with _cursor() as cur:
                ...
                if session_action == "open":
                    ...
                    cur.execute("""
                        INSERT INTO vc_sessions
                            (user_id, user_name, channel_id, channel_name, started_at, reservation_id)
                        VALUES (%s, %s, %s, %s, %s, %s)
                    """, (user_id, user_name, to_channel_id, to_channel_name, at, reservation_id_for_session))
            ...
            record_error("log_event_full_with_session", e)
            return
...
async def on_ready():
    ...
    try:
        if not hasattr(bot, "_doctor_expiry_task_started"):
            bot._doctor_expiry_task_started = True
            asyncio.create_task(temp_fix_expiry_loop())
    except Exception as e:
        print(f"[doctor] Failed to start temp-fix expiry loop (non-fatal): {e}", flush=True)
    try:
        await restore_reservation_views()
        await ensure_reservation_panel_posted()
        if not hasattr(bot, "_reservation_expiry_task_started"):
            bot._reservation_expiry_task_started = True
            asyncio.create_task(reservation_expiry_loop())
    except Exception as e:
        print(f"[reservations] Startup tasks failed (non-fatal): {e}", flush=True)
...
async def on_voice_state_update(member, before, after):
    ...
    # ---------- Reservation-scoped lock enforcement ----------
    # Deliberately separate from the global admin lock system above: that
    # system's locked_members table has a hard one-user-one-channel DB
    # constraint, which doesn't fit a user potentially belonging to several
    # different reservations. This check is entirely derived live from
    # reservations/reservation_members on every event instead — meaning
    # there's zero extra in-memory state to lose on a restart.
    if after_id is not None and not is_dashboard_action:
        reservation = await find_active_reservation_for_channel(after_id)
        if reservation and reservation["lock_state"] == "locked":
            with _cursor() as cur:
                cur.execute(
                    "SELECT 1 FROM reservation_members WHERE reservation_id = %s AND user_id = %s",
                    (reservation["id"], str(member.id)),
                )
                allowed = cur.fetchone() is not None
            if not allowed:
                try:
                    await member.move_to(None, reason=f"Reservation #{reservation['id']} is locked")
                except discord.Forbidden:
                    pass
                try:
                    await member.send("\U0001F6AB This VC is currently private.\nYou are not included in this reservation.")
                except discord.Forbidden:
                    pass
                log_event_full(
                    "LOCK_VIOLATION", member=member, to_channel=after.channel,
                    details=f"Not a member of reservation #{reservation['id']} (locked)",
                )
                return
    ...
    if before_id is None and after_id is not None:
        ...
        reservation = await find_active_reservation_for_channel(after.channel.id)
        if reservation and reservation["status"] == "approved":
            await activate_reservation_on_first_join(reservation)
        return
    ...
    if before_id is not None and after_id is None:
        ...
        with _cursor() as cur:
            cur.execute("SELECT status FROM temp_vc_pool WHERE channel_id = %s", (str(before.channel.id),))
            pool_row = cur.fetchone()
        if pool_row and rl.should_recycle_temp_vc(len(before.channel.members), pool_row["status"]):
            await recycle_temp_vc(member.guild, before.channel.id)
        return
...
def make_bot():
    ...
    @client.tree.command(name="my-profile", description="Get a link to your personal VC profile page on the website")
    async def my_profile_cmd(interaction: discord.Interaction):
        base = os.environ.get("SITE_BASE_URL", "").rstrip("/")
        path = f"/profile/{interaction.user.id}"
        if base:
            await interaction.response.send_message(f"\U0001F464 Here's your profile: {base}{path}", ephemeral=True)
        else:
            await interaction.response.send_message(
                f"\U0001F464 Your profile path is `{path}` \u2014 add it to your site's URL. "
                f"(An admin can set `SITE_BASE_URL` so this command gives the full link directly.)",
                ephemeral=True,
            )
    ...
    @client.tree.command(name="set-reservation-channel", description="Set the channel where the VC reservation booking panel posts (admin only)")
    @app_commands.describe(channel="The channel members will book reservations in")
    @app_commands.default_permissions(administrator=True)
    async def set_reservation_channel_cmd(interaction: discord.Interaction, channel: discord.TextChannel):
        set_reservation_booking_channel_id(channel.id)
        await ensure_reservation_panel_posted()
        await interaction.response.send_message(f"Reservation panel will now post in {channel.mention}.", ephemeral=True)

    @client.tree.command(name="set-reservation-approval-channel", description="Set the channel where admins review reservation requests (admin only)")
    @app_commands.describe(channel="The channel admins will see approval requests in")
    @app_commands.default_permissions(administrator=True)
    async def set_reservation_approval_channel_cmd(interaction: discord.Interaction, channel: discord.TextChannel):
        set_reservation_approval_channel_id(channel.id)
        await interaction.response.send_message(f"Reservation approval requests will now post in {channel.mention}.", ephemeral=True)

    @client.tree.command(name="post-reservation-panel", description="Post the VC reservation booking panel now (admin only)")
    @app_commands.default_permissions(administrator=True)
    async def post_reservation_panel_cmd(interaction: discord.Interaction):
        if not get_reservation_booking_channel_id():
            await interaction.response.send_message("Set a booking channel first with /set-reservation-channel.", ephemeral=True)
            return
        await ensure_reservation_panel_posted()
        await interaction.response.send_message("Done.", ephemeral=True)

    @client.tree.command(name="create-temp-vc-pool-channel", description="Create a new hidden reusable temp VC for the reservation pool (admin only)")
    @app_commands.describe(category="The category to create it under")
    @app_commands.default_permissions(administrator=True)
    async def create_temp_vc_pool_channel_cmd(interaction: discord.Interaction, category: discord.CategoryChannel):
        with _cursor() as cur:
            cur.execute("SELECT COUNT(*) AS n FROM temp_vc_pool")
            existing_count = cur.fetchone()["n"]
        label = f"{TEMP_VC_POOL_HIDDEN_NAME_PREFIX} {existing_count + 1:02d}"
        overwrites = {
        ...
        try:
            channel = await interaction.guild.create_voice_channel(label, category=category, overwrites=overwrites, reason="New temp VC reservation pool channel")
        except discord.Forbidden:
            await interaction.response.send_message("I don't have permission to create channels in that category.", ephemeral=True)
            return
        register_temp_vc_pool_channel(channel.id, label)
        await interaction.response.send_message(f"\u2705 Created and registered {channel.mention} as a pool channel.", ephemeral=True)

    @client.tree.command(name="reservation-settings", description="View current VC reservation spam-control settings (admin only)")
    @app_commands.default_permissions(administrator=True)
    async def reservation_settings_cmd(interaction: discord.Interaction):
        config = get_reservation_spam_config()
        lines = [f"**{k}**: {v}" for k, v in config.items()]
        booking_ch = get_reservation_booking_channel_id()
        approval_ch = get_reservation_approval_channel_id()
        lines.append(f"**booking_channel**: {f'<#{booking_ch}>' if booking_ch else 'not set'}")
        lines.append(f"**approval_channel**: {f'<#{approval_ch}>' if approval_ch else 'not set'}")
        embed = discord.Embed(title="\U0001F6E1\uFE0F VC Reservation Settings", description="\n".join(lines), color=discord.Color.blurple())
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @client.tree.command(name="set-reservation-limit", description="Adjust a VC reservation spam-control number (admin only)")
    @app_commands.describe(setting="Which setting to change", value="The new value")
    @app_commands.choices(setting=[
        app_commands.Choice(name="cooldown_minutes", value="cooldown_minutes"),
        app_commands.Choice(name="max_pending", value="max_pending"),
        app_commands.Choice(name="max_per_day", value="max_per_day"),
        app_commands.Choice(name="max_active", value="max_active"),
        app_commands.Choice(name="expire_after_hours", value="expire_after_hours"),
    ])
    @app_commands.default_permissions(administrator=True)
    async def set_reservation_limit_cmd(interaction: discord.Interaction, setting: app_commands.Choice[str], value: float):
        if value < 0:
            await interaction.response.send_message("Value can't be negative.", ephemeral=True)
            return
        caster = int if setting.value != "cooldown_minutes" and setting.value != "expire_after_hours" else float
        set_reservation_spam_config_value(setting.value, caster(value))
        await interaction.response.send_message(f"\u2705 {setting.value} set to {value:g}.", ephemeral=True)

    @client.tree.command(name="toggle-reservation-setting", description="Toggle a VC reservation on/off setting (admin only)")
    @app_commands.choices(setting=[
        app_commands.Choice(name="spam_protection_enabled", value="spam_protection_enabled"),
        app_commands.Choice(name="duplicate_protection_enabled", value="duplicate_protection_enabled"),
        app_commands.Choice(name="auto_expire_enabled", value="auto_expire_enabled"),
    ])
    @app_commands.default_permissions(administrator=True)
    async def toggle_reservation_setting_cmd(interaction: discord.Interaction, setting: app_commands.Choice[str], enabled: bool):
        set_reservation_spam_config_value(setting.value, enabled)
        await interaction.response.send_message(f"\u2705 {setting.value} is now {'ON' if enabled else 'OFF'}.", ephemeral=True)

    @client.tree.command(name="reservation-set-lock", description="Override a reservation's lock, even if the owner set it (admin only)")
    @app_commands.describe(reservation_id="The reservation number, e.g. from its approval message footer")
    @app_commands.choices(state=[
        app_commands.Choice(name="locked", value="locked"),
        app_commands.Choice(name="unlocked", value="unlocked"),
    ])
    @app_commands.default_permissions(administrator=True)
    async def reservation_set_lock_cmd(interaction: discord.Interaction, reservation_id: int, state: app_commands.Choice[str]):
        reservation = get_reservation(reservation_id)
        if reservation is None:
            await interaction.response.send_message("No reservation with that number.", ephemeral=True)
            return
        if reservation["status"] not in ("approved", "active"):
            await interaction.response.send_message(
                f"Reservation #{reservation_id} is {reservation['status']} — lock only applies to an approved/active reservation.",
                ephemeral=True,
            )
            return
        if reservation["lock_state"] == state.value:
            await interaction.response.send_message(f"Reservation #{reservation_id} is already {state.value}.", ephemeral=True)
            return

        # The owner's DM'd ReservationControlView isn't touched here — its
        # message_id was never persisted, so there's no message to edit in
        # place. It stays valid: it always reads live lock_state on toggle
        # (see ReservationControlView.toggle_lock), so the next time the
        # owner opens/clicks it, it'll reflect this admin override correctly.
        # Enforcement itself (find_active_reservation_for_channel, in
        # on_voice_state_update) reads lock_state live on every join too, so
        # this takes effect immediately — no Discord channel permissions to
        # touch.
        set_reservation_lock_state(reservation_id, state.value)
        vc_label = reservation["assigned_channel_name"] or reservation["reservation_name"]
        status_emoji = "\U0001F512" if state.value == "locked" else "\U0001F513"
        await interaction.response.send_message(
            f"{status_emoji} Reservation #{reservation_id} ({vc_label}) is now **{state.value}**.", ephemeral=True
        )

        try:
            requester = bot.get_user(int(reservation["requester_id"])) or await bot.fetch_user(int(reservation["requester_id"]))
            note = (
                "\U0001F512 An admin has locked your VC — only your reservation's members can join now."
                if state.value == "locked" else
                "\U0001F513 An admin has unlocked your VC — anyone can join now."
            )
            await requester.send(f"{note}\n({vc_label}, reservation #{reservation_id})")
        except (discord.Forbidden, discord.NotFound):
            pass  # requester has DMs closed or left the server — the lock change still applies

    @client.tree.command(name="reservation-stats", description="See time breakdown for a specific reservation")
    @app_commands.describe(reservation_id="The reservation number, e.g. from its approval message footer")
    async def reservation_stats_cmd(interaction: discord.Interaction, reservation_id: int):
        reservation = get_reservation(reservation_id)
        if reservation is None:
            await interaction.response.send_message("No reservation with that number.", ephemeral=True)
            return
        with _cursor() as cur:
            cur.execute("""
                SELECT user_id, user_name, channel_name, started_at, ended_at
                FROM vc_sessions WHERE reservation_id = %s ORDER BY started_at ASC
            """, (reservation_id,))
            raw_sessions = [
                {"user_id": r["user_id"], "user_name": r["user_name"], "channel_name": r["channel_name"],
                 "start": r["started_at"], "end": r["ended_at"]}
        ...
        clipped = vs.clip_sessions_to_period(raw_sessions, None, None)
        totals = vs.vc_time_totals(clipped, top=25)

        type_icon = "\U0001F512" if reservation["reservation_type"] == "private_vc" else "\U0001F50A"
        vc_label = reservation["assigned_channel_name"] or reservation["reservation_name"]
        start_local = reservation["start_time"] + timedelta(hours=TIMEZONE_OFFSET_HOURS)
        total_seconds = sum(s["duration_seconds"] for s in clipped)

        embed = discord.Embed(title=f"\U0001F4CA {type_icon} {vc_label}", color=discord.Color.teal())
        embed.add_field(name="Created by", value=reservation["requester_name"], inline=True)
        embed.add_field(name="Date", value=start_local.strftime("%d %B %Y"), inline=True)
        embed.add_field(name="Duration", value=vs.format_duration(total_seconds), inline=True)
        if totals:
            embed.add_field(
                name="Members",
                value="\n".join(f"{t['user_name']} \u2014 {vs.format_duration(t['total_seconds'])}" for t in totals),
                inline=False,
            )
        embed.add_field(name="Mode", value="\U0001F512 Locked" if reservation["lock_state"] == "locked" else "\U0001F513 Unlocked", inline=True)
        embed.set_footer(text=f"Reservation #{reservation_id}")
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @client.tree.command(name="doctor-mode", description="Run a full VC system health diagnosis (admin only)")
    @app_commands.default_permissions(administrator=True)
    async def doctor_mode_cmd(interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        issues = await run_doctor_diagnosis()
        report_text = format_doctor_report(issues)

        # Only propose a temporary fix for the one concrete, safe case this
        # version handles: one component dominating recent runtime errors.
        # Proposing (not applying) is enough here — a human always decides.
        view = None
        error_issue = next((i for i in issues if i["component"] == "Runtime Exceptions" and i["severity"] == "HIGH"), None)
        if error_issue:
            recent_cutoff = datetime.now(timezone.utc) - timedelta(hours=1)
            by_component = defaultdict(int)
            for e in recent_errors:
                if e["time"] >= recent_cutoff:
                    by_component[e["component"]] += 1
            worst_component = max(by_component, key=by_component.get) if by_component else None
            toggle_map = {"log_event_full_with_session": "gambling", "fetch_sessions_for_period": "gambling"}
            toggle_key = toggle_map.get(worst_component)
            if worst_component and toggle_key:
                fix_id = propose_temp_fix(
                    component=worst_component,
                    reason=f"{by_component[worst_component]} errors in the last hour",
                    toggle_key=toggle_key,
                )
                view = DoctorFixProposalView(fix_id, worst_component, toggle_key)
                report_text += (
                    f"\n\n\U0001F691 **Proposed temporary fix available** (Fix ID: {fix_id}) — "
                    f"nothing applied yet, see buttons below."
                )

        # Immediate confirmation to whoever ran it...
        await interaction.followup.send(
            "\u2705 Diagnosis complete — full report DMed to all admins." if len(ANNOUNCE_ADMIN_IDS) > 1
            else "\u2705 Diagnosis complete — see your DMs for the full report.",
            ephemeral=True,
        )
        # ...and the actual report goes to EVERY admin's DMs, not just theirs.
        await send_doctor_report_to_admins(report_text, view=view)

    @client.tree.command(name="run-smart-recovery", description="Manually run Smart Recovery now and see the report (admin only)")
    @app_commands.default_permissions(administrator=True)
    async def run_smart_recovery_cmd(interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        report = await reconcile_vc_sessions(send_report=False)
        if report is None:
            await interaction.followup.send("Smart Recovery couldn't run — database or guild not available.", ephemeral=True)
            return
        if not report["actionable"] and not report["conflicts"]:
            await interaction.followup.send(
                f"\u2705 Everything matches — {report['match_count']} session(s) checked, nothing to repair.",
                ephemeral=True,
            )
            return

        guild = interaction.guild
        lines = [f"\U0001F6E1\uFE0F **SMART RECOVERY** ({report['match_count']} match, {len(report['actionable'])} repaired, {len(report['conflicts'])} conflict(s))", ""]
        category_labels = {"missing": "Stale session closed", "mismatch": "Channel reconciled", "new": "Session opened"}
        for r in report["actionable"]:
            member = guild.get_member(int(r["user_id"]))
            name = member.display_name if member else r["user_id"]
            lines.append(f"**{name}** \u2014 DB: {r['db_channel_name'] or '—'} / Discord: {r['discord_channel_name'] or '—'} \u2192 {category_labels[r['category']]}")
        if report["conflicts"]:
            lines.append("")
            lines.append("\u26A0\uFE0F Conflicts needing manual review (not auto-fixed):")
            for uid in report["conflicts"]:
                member = guild.get_member(int(uid))
...
@app.route("/api/reservation-settings", methods=["GET"])
def api_reservation_settings_get():
    if not check_auth():
        return jsonify({"error": "unauthorized"}), 401
    return jsonify(get_reservation_spam_config())


@app.route("/api/reservation-settings", methods=["POST"])
def api_reservation_settings_set():
    if not check_auth():
        return jsonify({"error": "unauthorized"}), 401
    body = request.get_json(silent=True) or {}
    numeric_fields = {"cooldown_minutes": float, "max_pending": int, "max_per_day": int, "max_active": int, "expire_after_hours": float}
    boolean_fields = ("spam_protection_enabled", "duplicate_protection_enabled", "auto_expire_enabled")
    try:
        for field, caster in numeric_fields.items():
            if field in body:
                value = caster(body[field])
                if value < 0:
                    return jsonify({"error": f"{field} can't be negative"}), 400
                set_reservation_spam_config_value(field, value)
        for field in boolean_fields:
            if field in body:
                set_reservation_spam_config_value(field, bool(body[field]))
    except (TypeError, ValueError):
        return jsonify({"error": "invalid value"}), 400
    return jsonify(get_reservation_spam_config())
...
def api_profile(user_id):
    # (no password required — this is a read-only endpoint, same precedent as /api/leaderboard.
    # Profile protection is explicitly on hold — not implemented yet, by design.)
    if not is_enabled():
    ...
    def total_for(period):
        ...
        raw = fetch_sessions_for_period(p_start, p_end)
        clipped = vs.clip_sessions_to_period(raw, p_start, p_end, now=now)
        return sum(s["duration_seconds"] for s in clipped if s["user_id"] == user_id)

    today_secs = total_for("today")
    week_secs = total_for("week")
    month_secs = total_for("month")

    all_sessions = [s for s in fetch_sessions_for_period(None, None) if s["user_id"] == user_id]
    all_clipped = vs.clip_sessions_to_period(all_sessions, None, None, now=now)
    total_secs = sum(s["duration_seconds"] for s in all_clipped)
    longest = vs.longest_sessions(all_clipped, top=1)
    longest_secs = longest[0]["duration_seconds"] if longest else 0
    channels = vs.channel_totals(all_clipped, top=1)
    most_used = channels[0]["channel_name"] if channels else None

    if display_name is None and not all_clipped:
        return jsonify({"error": "no such member or no VC history"}), 404

    return jsonify({
        "user_id": user_id,
        "display_name": display_name or f"User {user_id}",
        "avatar_url": avatar_url,
        "sessions_count": len(all_clipped),
        "most_used_channel": most_used,
        "seconds": {
            "total": total_secs, "today": today_secs, "week": week_secs,
            "month": month_secs, "longest_session": longest_secs,
        },
        "formatted": {
            "total": vs.format_duration(total_secs), "today": vs.format_duration(today_secs),
            "week": vs.format_duration(week_secs), "month": vs.format_duration(month_secs),
            "longest_session": vs.format_duration(longest_secs),
        },
    })
...
def profile_page(user_id):
    return render_template_string(PROFILE_HTML, user_id=user_id)
...
def start_bot():
    ...
    while True:
        ...
        try:
            ...
            if status == 429:
                # Discord tells us exactly how long to back off in the Retry-After
                # header (this can be minutes, well beyond our own exponential
                # backoff schedule). Ignoring it and retrying early looks like
                # continued abuse to Discord's rate limiter and can make the
                # block escalate — so if it's present, it wins over retry_delay.
                header_wait = None
                ...
                if raw is not None:
                    try:
                        ...
                        header_wait = None
                wait = max(retry_delay, header_wait + 5) if header_wait is not None else retry_delay
                print(f"[bot] Discord rate-limited our login (429). Retrying in {wait}s...", flush=True)
            else:
                wait = retry_delay
                print(f"[bot] Discord HTTP error during login ({status}): {e}. Retrying in {wait}s...", flush=True)
        except Exception as e:
            wait = retry_delay
            print(f"[bot] Unexpected error connecting to Discord: {e}. Retrying in {wait}s...", flush=True)
        finally:
