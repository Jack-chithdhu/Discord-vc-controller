...
async function refreshCurrentTab() {
  ...
  else if (currentTab === 'settings') { loadToggles(); }
}
...
def init_db():
    ...
    try:
        ...
        with _cursor() as cur:
            ...
            """)
        print("[db] PostgreSQL ready (vc_logs, vc_lock_channels, vc_lock_members, join_requests, bot_settings, birthdays, points).", flush=True)
        return True
...
async def nudge_member_for_birthday(member: discord.Member):
    try:
        await member.send(
        ...
        pass  # DMs closed — nothing more we can do


async def backfill_birthday_nudges():
    """One-time-per-startup sweep: DMs the birthday button to existing
    members who don't have one on file yet. Controlled by its own toggle
    so it can run independently of new-member nudges."""
    ...
    for member in guild.members:
        if member.bot or has_birthday_registered(member.id):
            continue
        await nudge_member_for_birthday(member)
        sent += 1
        await asyncio.sleep(1)  # gentle pacing to avoid DM rate limits on larger servers
    if sent:
        print(f"[birthday] Backfill-nudged {sent} existing member(s) without a birthday on file.", flush=True)
...
MAX_OPEN_SESSION_HOURS = 8  # a session with no closing event this old is almost
                            # certainly stale (bot was offline when they actually
                            # left), not someone genuinely still in voice — cap it
                            # here instead of letting it grow forever on every refresh
...
def reconstruct_sessions_with_ids(events, now=None):
    ...
    for uid, s in open_sessions.items():
        ...
        cap = s["start"] + timedelta(hours=MAX_OPEN_SESSION_HOURS)
        close_at = min(now, cap)
        sessions.append({**_close(s, close_at), "user_id": uid})

    return sessions


def _filter_period(sessions, period, now=None, tz_offset_hours=0):
    """Keeps sessions that ended on/after the start of the requested period,
    measured in LOCAL time (tz_offset_hours added to UTC) so "today"/"week"/"month"
    match the user's actual calendar day, not the UTC one."""
    now = now or datetime.now(timezone.utc)
    if period == "all":
        return sessions
    from datetime import timedelta
    ...
    if period == "today":
        ...
        local_cutoff = local_now.replace(hour=0, minute=0, second=0, microsecond=0)
        local_cutoff = local_cutoff.fromordinal(local_cutoff.toordinal() - local_cutoff.weekday())
    elif period == "month":
        local_cutoff = local_now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    else:
        return sessions
    utc_cutoff = local_cutoff - timedelta(hours=tz_offset_hours)
    return [s for s in sessions if s["end"] >= utc_cutoff]


def vc_time_totals(sessions, period="all", now=None, top=10, tz_offset_hours=0):
    """Total VC time per user within the period, sorted descending."""
    filtered = _filter_period(sessions, period, now, tz_offset_hours)
...
def longest_sessions(sessions, period="all", now=None, top=10, tz_offset_hours=0):
...
def _shift_session(session, tz_offset_hours):
    """Return a copy of the session with start/end shifted by the timezone offset,
    so hour-of-day bucketing reflects local time instead of UTC. Duration is
    unaffected since both endpoints shift by the same amount."""
...
def night_owl_and_early_bird(sessions, period="all", now=None, tz_offset_hours=0):
    """
    Night owl: most total time logged between 22:00-04:00 LOCAL time.
    Early bird: most total time logged between 05:00-09:00 LOCAL time.
    tz_offset_hours: hours to add to UTC to get local time (e.g. 5.5 for IST).
...
def hour_of_day_totals(sessions, period="all", now=None, tz_offset_hours=0):
    """Total combined VC-seconds per LOCAL hour-of-day (0-23), across all users. For a bar chart.
    tz_offset_hours: hours to add to UTC to get local time (e.g. 5.5 for IST)."""
    from datetime import timedelta
    filtered = _filter_period(sessions, period, now, tz_offset_hours)
    totals = [0.0] * 24
...
def channel_totals(sessions, period="all", now=None, top=10, tz_offset_hours=0):
...
async def reconcile_stale_sessions():
    ...
    there and keep extending their time forever (capped at MAX_OPEN_SESSION_HOURS,
    but still wrong). This compares who the logs *think* is still in voice against
    who's actually there right now, and closes out anyone who isn't — so their
    time stops at roughly when the bot went down, not keeps growing indefinitely.
    """
    if not is_enabled():
        return
    guild = bot.get_guild(GUILD_ID)
...
def make_bot():
    ...
    async def daily_cmd(interaction: discord.Interaction):
        if not get_toggle("gambling"):
            ...
            return
        await interaction.response.send_message(
            f"Tap to claim your daily {DAILY_CLAIM_AMOUNT} points:",
    ...
    async def balance_cmd(interaction: discord.Interaction):
        if not get_toggle("gambling"):
            await interaction.response.send_message("The points system is turned off right now.", ephemeral=True)
            return
        await interaction.response.send_message(f"Balance: {get_balance(interaction.user.id)} points", ephemeral=True)
...
def api_leaderboard():
    ...
    try:
        ...
        vc_time = vc_time_totals(sessions, period=period, tz_offset_hours=TIMEZONE_OFFSET_HOURS)
        longest = longest_sessions(sessions, period=period, tz_offset_hours=TIMEZONE_OFFSET_HOURS)
        night_early = night_owl_and_early_bird(sessions, period=period, tz_offset_hours=TIMEZONE_OFFSET_HOURS)
...
def api_analytics_dashboard():
    ...
    try:
        events = fetch_events_since(days_back=90)
        sessions = reconstruct_sessions_with_ids(events)
        hour_totals = hour_of_day_totals(sessions, period=period, tz_offset_hours=TIMEZONE_OFFSET_HOURS)
        channels = channel_totals(sessions, period=period, tz_offset_hours=TIMEZONE_OFFSET_HOURS)
