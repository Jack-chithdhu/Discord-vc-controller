...
def make_bot():
    ...
    async def doctor_mode_cmd(interaction: discord.Interaction):
        ...
        for chunk_start in range(0, len(report_text), 1900):
            chunk = report_text[chunk_start:chunk_start + 1900]
            if chunk_start == 0:
                await interaction.followup.send(chunk, view=view, ephemeral=True)
            else:
