...
def create_reservation_request(guild_id, requester_id, requester_name, reservation_name,
                                ...
                                start_time, end_time, reason, member_ids_and_names=None):
    """
    ...
    with _cursor() as cur:
        cur.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'pending', 'locked')
            RETURNING id
...
async def _submit_booking(interaction: discord.Interaction, *, reservation_type: str,
                           ...
                           start_utc, end_utc, members_text: str, reason: str):
    """Shared submission path for both booking modals — validates via
    ...
    member_ids = rl.parse_member_ids(members_text)
    ...
    ok, result = create_reservation_request(
        ...
        member_ids_and_names=resolved_members,
    )
    if not ok:
...
class ExistingVCBookingModal(discord.ui.Modal, title="Reserve an Existing VC"):
    date_start = discord.ui.TextInput(label="Start (YYYY-MM-DD HH:MM)", placeholder="2026-09-20 19:30")
    end_time = discord.ui.TextInput(label="End time (HH:MM, same day)", placeholder="21:30", max_length=5)
    members = discord.ui.TextInput(label="Members (@mentions or IDs)", style=discord.TextStyle.paragraph, required=False)
    reason = discord.ui.TextInput(label="Reason (optional)", required=False, max_length=200)
...
class ExistingVCChannelSelect(discord.ui.Select):
    def __init__(self):
        ...
        super().__init__(
            placeholder="Choose a VC...", options=options or [discord.SelectOption(label="No voice channels found", value="none")],
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if channel is None:
            ...
            return
        await interaction.response.send_modal(ExistingVCBookingModal(channel.id, channel.name))


class ExistingVCChannelSelectView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=120)
        self.add_item(ExistingVCChannelSelect())
...
class ReservationBookingPanelView(discord.ui.View):
    ...
    async def existing_vc(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not get_toggle("reservations"):
            ...
            return
        await interaction.response.send_message("Choose a VC:", view=ExistingVCChannelSelectView(), ephemeral=True)
