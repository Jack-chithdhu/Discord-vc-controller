...
def expire_stale_pending_reservations() -> int:
    """Auto-expires PENDING reservations older than the configured
    threshold, if reservation_auto_expire_enabled is on. Returns how many
    were expired. The expiry check itself is pure (reservation_logic.
    is_pending_expired) — this just applies it and writes the result."""
    config = get_reservation_spam_config()
