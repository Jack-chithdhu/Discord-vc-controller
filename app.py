...
function updatePwboxVisibility() {
  ...
  if (!box) return;
  const needsPassword = currentTab === 'teams' || currentTab === 'lock';
...
def init_db():
    ...
    try:
        ...
        with _cursor() as cur:
            ...
            """)
        print("[db] PostgreSQL ready (vc_logs, vc_lock_channels, vc_lock_members, join_requests, bot_settings).", flush=True)
        return True
...
class JoinApprovalView(discord.ui.View):
    ...
    async def hold(self, interaction: discord.Interaction, button: discord.ui.Button):
        with _cursor() as cur:
            cur.execute("UPDATE join_requests SET status = 'on_hold' WHERE id = %s", (self.req_id,))

        embed = interaction.message.embeds[0]
        embed.color = discord.Color.light_grey()
        embed.set_footer(text=f"Request #{self.req_id} — on hold")
    ...
    async def _resolve(self, interaction: discord.Interaction, status: str):
        with _cursor() as cur:
            cur.execute("UPDATE join_requests SET status = %s WHERE id = %s", (status, self.req_id))
        ...
        try:
            ...
            if status == "approved":
                await member.send(f"You're approved! Join here: {get_whatsapp_link()}")
            else:
...
_raw_announce_channel_id = os.environ.get("ANNOUNCEMENT_CHANNEL_ID", "0")  # legacy — no longer used, kept for reference
