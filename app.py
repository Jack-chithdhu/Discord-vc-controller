...
async def reservation_expiry_loop():
    """Periodically expires stale PENDING reservations and edits their
    approval message to show it's no longer actionable — avoids a pending
    request sitting forever with live buttons nobody will ever click."""
    await bot.wait_until_ready()
    while not bot.is_closed():
        try:
