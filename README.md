...
- **Activity log** at the bottom shows everyone who joined, left, or got moved between any voice channel on the server — including moves triggered by the site itself — auto-refreshing every 10s. This only tracks activity from when the bot is online; it resets when the app restarts (Render free tier restarts on redeploy or after being idle then woken).
