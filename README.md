...
- Everyone joins the Lobby VC → the player list loads automatically → **tap each name** to assign them to Team A or Team B (tap again to switch, a third tap clears it)
- Want it random instead? Tap **🎲 Random assign** to auto-split, then still tap any name to override before moving
- Tap **Move → Team A** / **Move → Team B** to actually move whoever's assigned to that team
- After the round: **Regroup to Lobby**, or **Rematch** to swap whoever's currently in Team A/B channels with each other
- If you add/rename VCs later, tap **↻ Refresh channel list**; if people join/leave the lobby, tap **↻ Refresh players**
