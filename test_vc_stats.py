...
def test_who_was_with_me_basic_overlap():
    """The exact example from the spec: A 10:00-11:00, B 10:20-10:45, C 10:40-11:30 — same channel."""
    sessions = [
        {"user_id": "A", "user_name": "Alice", "channel_name": "Gaming",
         "start": dt(2026, 1, 1, 10, 0), "end": dt(2026, 1, 1, 11, 0)},
        {"user_id": "B", "user_name": "Bob", "channel_name": "Gaming",
         "start": dt(2026, 1, 1, 10, 20), "end": dt(2026, 1, 1, 10, 45)},
        {"user_id": "C", "user_name": "Carol", "channel_name": "Gaming",
         "start": dt(2026, 1, 1, 10, 40), "end": dt(2026, 1, 1, 11, 30)},
    ]
    result = vs.compute_who_was_with_me("A", sessions)
    by_id = {r["user_id"]: r["seconds"] for r in result}
    assert by_id["B"] == 25 * 60   # 10:20-10:45
    assert by_id["C"] == 20 * 60   # 10:40-11:00 (A leaves at 11:00, not 11:30)


def test_who_was_with_me_different_channels_dont_count():
    """Same time window, different VCs — should NOT count as together."""
    sessions = [
        {"user_id": "A", "user_name": "Alice", "channel_name": "Gaming",
         "start": dt(2026, 1, 1, 10, 0), "end": dt(2026, 1, 1, 11, 0)},
        {"user_id": "B", "user_name": "Bob", "channel_name": "Music",
         "start": dt(2026, 1, 1, 10, 0), "end": dt(2026, 1, 1, 11, 0)},
    ]
    result = vs.compute_who_was_with_me("A", sessions)
    assert result == []
...
def test_who_was_with_me_multiple_sessions_and_moves():
    """A moves Gaming->Music; B is in Gaming then later also moves to Music —
    overlap should be summed correctly across the separate per-channel sessions."""
    sessions = [
        {"user_id": "A", "user_name": "Alice", "channel_name": "Gaming",
         "start": dt(2026, 1, 1, 10, 0), "end": dt(2026, 1, 1, 10, 30)},
        {"user_id": "A", "user_name": "Alice", "channel_name": "Music",
         "start": dt(2026, 1, 1, 10, 30), "end": dt(2026, 1, 1, 11, 0)},
        {"user_id": "B", "user_name": "Bob", "channel_name": "Gaming",
         "start": dt(2026, 1, 1, 10, 10), "end": dt(2026, 1, 1, 10, 30)},
        {"user_id": "B", "user_name": "Bob", "channel_name": "Music",
         "start": dt(2026, 1, 1, 10, 30), "end": dt(2026, 1, 1, 10, 50)},
    ]
    result = vs.compute_who_was_with_me("A", sessions)
    by_id = {r["user_id"]: r["seconds"] for r in result}
    assert by_id["B"] == 20 * 60 + 20 * 60  # 10:10-10:30 in Gaming + 10:30-10:50 in Music


def test_who_was_with_me_open_session_counts_to_now():
    """An other user still in VC (end=None) should count up to `now`, not be skipped."""
    sessions = [
        {"user_id": "A", "user_name": "Alice", "channel_name": "Gaming",
         "start": dt(2026, 1, 1, 10, 0), "end": dt(2026, 1, 1, 11, 0)},
        {"user_id": "B", "user_name": "Bob", "channel_name": "Gaming",
         "start": dt(2026, 1, 1, 10, 30), "end": None},
    ]
    now = dt(2026, 1, 1, 10, 45)
    result = vs.compute_who_was_with_me("A", sessions, now=now)
    by_id = {r["user_id"]: r["seconds"] for r in result}
    assert by_id["B"] == 15 * 60  # 10:30 -> 10:45 (now), not all the way to A's 11:00 end


def test_who_was_with_me_no_overlap_excluded():
    """Sessions that don't actually overlap at all shouldn't appear in the results."""
    sessions = [
        {"user_id": "A", "user_name": "Alice", "channel_name": "Gaming",
         "start": dt(2026, 1, 1, 10, 0), "end": dt(2026, 1, 1, 10, 30)},
        {"user_id": "B", "user_name": "Bob", "channel_name": "Gaming",
         "start": dt(2026, 1, 1, 10, 30), "end": dt(2026, 1, 1, 11, 0)},  # back-to-back, not overlapping
    ]
    result = vs.compute_who_was_with_me("A", sessions)
    assert result == []


# ---------- 12. Smart Recovery classification (Match/Missing/Mismatch/New) ----------

def test_classify_match():
    records = vs.classify_vc_state(open_sessions={"u1": "chA"}, currently_present={"u1": "chA"})
    assert records == [{"user_id": "u1", "category": "match", "db_channel": "chA", "discord_channel": "chA"}]


def test_classify_missing():
    records = vs.classify_vc_state(open_sessions={"u1": "chA"}, currently_present={})
    assert records == [{"user_id": "u1", "category": "missing", "db_channel": "chA", "discord_channel": None}]


def test_classify_mismatch():
    records = vs.classify_vc_state(open_sessions={"u1": "chA"}, currently_present={"u1": "chB"})
    assert records == [{"user_id": "u1", "category": "mismatch", "db_channel": "chA", "discord_channel": "chB"}]


def test_classify_new():
    records = vs.classify_vc_state(open_sessions={}, currently_present={"u1": "chA"})
    assert records == [{"user_id": "u1", "category": "new", "db_channel": None, "discord_channel": "chA"}]


def test_classify_is_exhaustive_and_covers_everyone():
    """Every user_id appearing in either input gets exactly one record, including matches."""
    records = vs.classify_vc_state(
        open_sessions={"u1": "chA", "u2": "chA"},
        currently_present={"u1": "chA", "u3": "chB"},
    )
    by_uid = {r["user_id"]: r["category"] for r in records}
    assert by_uid == {"u1": "match", "u2": "missing", "u3": "new"}
    assert len(records) == 3


def test_classify_agrees_with_compute_reconciliation_actions():
    """compute_reconciliation_actions is now built on classify_vc_state — verify
    they never disagree, across a mixed scenario."""
    open_sessions = {"u1": "chA", "u2": "chA", "u3": "chA"}
    currently_present = {"u1": "chA", "u2": "chB", "u4": "chC"}
    records = vs.classify_vc_state(open_sessions, currently_present)
    to_close, to_open = vs.compute_reconciliation_actions(open_sessions, currently_present)

    expected_close = {r["user_id"] for r in records if r["category"] in ("missing", "mismatch")}
    expected_open = {r["user_id"] for r in records if r["category"] in ("new", "mismatch")}
    assert to_close == expected_close == {"u2", "u3"}
    assert to_open == expected_open == {"u2", "u4"}
